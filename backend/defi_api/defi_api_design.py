...
class DeFiFormulaAPI:
    ...
    def _load_formulas(self) -> List[Dict]:
        ...
        formulas = [
            ...
            },
            {
                ...
                'variables': [
                    {'name': 'p', 'description': 'Price ratio', 'unit': 'dimensionless', 'type': 'float'}
                ],
                'parameters': None,
    ...
    def _setup_routes(self):
        ...
        async def get_formula(
            formula_id: str,
            format: FormatType = Query(FormatType.STRUCTURED)
        ):
        ...
        async def search_formulas(
            ...
            results = [
                ...
                if query_lower in f['description'].lower()
            ][:limit]
        ...
        async def export_formulas(
            format: FormatType = Query(FormatType.CSV)
        ):
    ...
    def _to_minimal(self, formulas: List[Dict]) -> List[str]:
        ...
        return [
            f"{f['description']};{f['analytical_formula']}"
            for f in formulas
        ]
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        keywords = []
        defi_terms = ['pool', 'fee', 'swap', 'liquidity', 'AMM', 'impermanent', 'loss', 
                     'constant', 'product', 'reserve', 'token', 'price', 'yield', 'APY']

...
class HypatiaXService:
    ...
    def map_description_to_formula(
        ...
        try:
            ...
            logger.info(f"Mapped: '{description}' -> {result['formula']} (confidence={result.get('confidence', 0):.2f})")
    ...
    def _detect_domain(self, description: str) -> str:
        ...
        defi_keywords = [
            'impermanent loss', 'il', 'liquidity', 'pool', 'swap', 'amm',
            'token', 'defi', 'uniswap', 'fee', 'tvl', 'slippage', 'apy',
            'yield', 'reserve', 'price ratio', 'breakeven'
        ]
    ...
    def get_defi_formulas(self, category: Optional[str] = None) -> Dict[str, Dict]:
        ...
        if category:
            return {
                k: v for k, v in self.DEFI_FORMULAS.items()

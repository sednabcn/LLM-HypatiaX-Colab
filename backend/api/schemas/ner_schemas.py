...
class FormulaExtractionSchema(Schema):
    ...
    output_format = fields.Str(
        missing='json',
        validate=validate.OneOf(['json', 'latex', 'both'])
    )
...
class ExpressionParseSchema(Schema):
    ...
    output_format = fields.Str(
        missing='tree',
        validate=validate.OneOf(['tree', 'list', 'graph'])
    )
...
class LatexConversionSchema(Schema):
    ...
    style = fields.Str(
        missing='inline',
        validate=validate.OneOf(['inline', 'display', 'equation'])
    )

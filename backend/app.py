...
def mock_formula_generation(description, method='vocab'):
    ...
    for i, word in enumerate(words):
        if word.lower() in ['of', 'by', 'for']:
            if i + 1 < len(words):
                remaining = words[i+1:]
                field_words = [w for w in remaining if w.lower() not in 
                             ['by', 'per', 'for', 'the', 'a', 'an', 'in', 'on']]
                if field_words:
    ...
    if not field_name:
        for word in reversed(words):
            ...
            if clean_word not in ['sum', 'average', 'avg', 'count', 'total', 'max', 'min', 
                                  'of', 'by', 'the', 'a', 'an', 'calculate', 'compute']:
...
def hypatiax_batch():
    ...
    try:
        ...
        if not data or 'descriptions' not in data:
            return jsonify({
                'success': False,
                'error': 'descriptions field is required'
            }), 400

...
class ConfigModel(BaseModel):
    ground_truth: Optional[str] = Field(None, description="Ground truth equation (optional)")
...
class SymbolicRegressionEngine:
    ...
    async def run_discovery(self, callback=None):
        ...
        if self.config.ground_truth:
            ...
            expression = ' + '.join([f'{np.random.randn():.2f}*{v}' 
                                   for v in self.config.variables[:2]])
    ...
    def validate(self, expression: str) -> List[ValidationCheck]:
        ...
        checks.append(ValidationCheck(
            ...
            message="Expression discovered successfully" if expression else "Discovery failed"
        ))
...
async def get_job_status(job_id: str):
    ...
    return {
        'job_id': job_id,
        'status': job['status'],
        'progress': len(job.get('convergence_data', [])) / job['config']['generations'] * 100
                   if job.get('convergence_data') else 0
    }

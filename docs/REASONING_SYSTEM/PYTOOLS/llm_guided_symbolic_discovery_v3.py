    ...
    def _call_openai(self, prompt: str) -> str:
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert equation discovery system.",
                },
    ...
    def discover(
        ...
        for hyp in hypotheses:
            verified_hyp = self.verifier.verify(
                hyp, X, y, variable_names, variable_units=variable_units, domain=domain
            )
            ...
            if verified_hyp.r2_score is not None:
                ...
                if verified_hyp.validation_score is not None:
                    val_status = (
                        "✅"
                        if verified_hyp.validation_score > validation_threshold
                        else "⚠️"
                    )
                    val_str = (
                        f" | Val: {val_status} {verified_hyp.validation_score:.1f}/100"
                    )
        ...
        needs_fallback = (
            ...
            or best_hypothesis.r2_score < success_threshold
            or (
                best_hypothesis.validation_score is not None
                and best_hypothesis.validation_score < validation_threshold
            )
        )
            ...
            if self.fallback_to_pysr:
                ...
                if best_hypothesis:
                    ...
                    if best_hypothesis.validation_score:
                        print(
                            f"   Best LLM Val = {best_hypothesis.validation_score:.1f}/100"
                        )
        ...
        if best_hypothesis:
            ...
            meets_val = (
                best_hypothesis.validation_score is None  # No validation = pass
                or best_hypothesis.validation_score > validation_threshold
            )
        ...
        if success:
            ...
            print(
                f"   Equation: {best_hypothesis.fitted_equation or best_hypothesis.equation}"
            )
            print(f"   R² Score: {best_hypothesis.r2_score:.4f}")
            if best_hypothesis.validation_score:
                val_status = (
                    "✅"
                    if best_hypothesis.validation_score > validation_threshold
                    else "⚠️"
                )
                print(
                    f"   Validation: {val_status} {best_hypothesis.validation_score:.1f}/100"
                )
            ...
            if best_hypothesis:
                ...
                if best_hypothesis.validation_score:
                    ...
                    print(
                        f"   Thresholds: R²>{success_threshold:.2f}, Val>{validation_threshold:.1f}"
                    )
        ...
        return {
            ...
            "thresholds": {"r2": success_threshold, "validation": validation_threshold},
        }
...
def test_kinetic_energy(api_key: Optional[str] = None):
    ...
    discoverer = LLMGuidedDiscovery(
        llm_provider="anthropic", api_key=api_key, fallback_to_pysr=False
    )
    ...
    result = discoverer.discover(
        ...
        variable_descriptions={"m": "Object mass in kg", "v": "Object velocity in m/s"},
...
def test_bernoulli(api_key: Optional[str] = None):
    ...
    result = discoverer.discover(
        ...
        variable_units={"P": "Pa", "rho": "kg/m^3", "v": "m/s", "g": "m/s^2", "h": "m"},
        n_hypotheses=5,
        success_threshold=0.95,
        validation_threshold=70.0,
    )
...
def test_michaelis_menten(api_key: Optional[str] = None):
    ...
    result = discoverer.discover(
        ...
        variable_units={"Vmax": "mol/(L*s)", "substrate": "mol/L", "Km": "mol/L"},
        n_hypotheses=5,
        success_threshold=0.95,
        validation_threshold=70.0,
    )
...
Note: You need an Anthropic API key. Get one at: https://console.anthropic.com/
    ...
    if args.test == "all":
        ...
        for name, result in results.items():
            ...
            val = (
                result["best_hypothesis"].validation_score
                if (
                    result["best_hypothesis"]
                    and result["best_hypothesis"].validation_score
                )
                else 0
            )
            print(
                f"{name:20s} {status:8s} R²={r2:.4f} Val={val:.1f}/100 Time={time_taken:.1f}s"
            )

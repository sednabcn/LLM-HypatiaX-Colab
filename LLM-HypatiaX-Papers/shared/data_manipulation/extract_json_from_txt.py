    ...
    if data_results_dir.exists():
        ...
        if json_files:
            ...
            if llm_files and nn_files:
                ...
                try:
                    with open(llm_best) as f:
                        ...
                        if isinstance(llm_data, list):
                            llm_domains = set(item.get('domain', 'unknown') for item in llm_data)
                ...
                try:
                    with open(nn_best) as f:
                        ...
                        if isinstance(nn_data, list):
                            nn_domains = set(item.get('domain', 'unknown') for item in nn_data)
                ...
                print("="*80)
                print("\nRun comparison with:")
                print(f"  python comparison_analysis_improved.py baseline_llm_ALL.json baseline_nn_ALL.json")

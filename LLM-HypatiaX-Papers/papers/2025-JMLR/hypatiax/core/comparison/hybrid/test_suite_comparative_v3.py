        ...
        for domain in protocol.get_all_domains():
            ...
            for desc, X, y, var_names, meta in test_cases:
                all_tests.append({
                    ...
                    "var_names": var_names,
                    "metadata": meta,
                    "why_wins": f"LLM understands {meta['formula_type']}, PySR refines"
                })
...
def run_five_strategic_tests(
    ...
    """
    Run strategic tests using experiment protocol
    
    Args:
        num_samples: Number of data points (from protocol, fixed at 200)
        verbose: Print detailed output
        domain_filter: Filter tests by domain
            - 'all': Run all tests
            - 'all_domains': Run scientific domain tests only
            - 'defi': Run DeFi tests only
    """
    
    suite = ComparativeTestSuite()
    tests_manager = FiveStrategicTests()
    
    # Get all tests from protocol
...
Examples:
  ...
  python test_suite_comparative.py --samples 200 --verbose --domain chemistry
    ...
    parser.add_argument('--domain', type=str, 
                        choices=['all', 'all_domains', 'defi', 'chemistry', 'biology'], 
                        default='all',

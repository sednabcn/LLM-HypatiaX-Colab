"""
HypatiaX Hybrid Discovery System v3.8 - FIXED AUTO-CONFIGURATION + CONFIGURABLE ITERATIONS
...
- NEW: Fully configurable iterations - no hardcoded values!
"""
...
class HybridDiscoverySystem:
    """
    Hybrid discovery system v3.8 with FIXED auto-configuration + configurable iterations.
    ...
    - NEW: No hardcoded iterations - fully controllable externally
    """
    ...
    def __init__(
        ...
        logger.info(f"=" * 70)
        logger.info(f"HybridDiscoverySystem v4.0 - FIXED AUTO-CONFIGURATION")
        logger.info(f"=" * 70)
        ...
        # Configure symbolic engine with provided config or defaults
        # NEW: Don't override if discovery_config is provided!
        if discovery_config is None:
            symbolic_config = DiscoveryConfig(
                niterations=100,  # Only used as fallback when no config provided
                enable_auto_configuration=enable_auto_config,
            )
            logger.info(f"Using default iterations: 100 (no config provided)")
        else:
            symbolic_config = discovery_config  # Use provided config as-is
    ...
    def _discover_with_retry(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # The iterations are already configured in self.symbolic_engine
                ...
                result["collapsedy_constants"] = collapsed
    ...
    def discover_validate_interpret(
        ...
        # ----------------------------------------------------------
        # Attach collapsed-constant warnings (post-validation)
        # ----------------------------------------------------------
        ...
        # ==========================================================
        # STEP B: DISCOVERY MODE ACCEPTANCE LOGIC
        # ==========================================================
...
if __name__ == "__main__":
    ...
    # NEW: Configurable iterations!
    discovery_config = DiscoveryConfig(
        niterations=60,  # Externally controllable!
        enable_auto_configuration=True,
    )

    system = HybridDiscoverySystem(
        domain="physics",
        discovery_config=discovery_config,  # Pass config

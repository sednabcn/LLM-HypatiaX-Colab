...
class Generation_custom_queries_patterns:
       def __init__(self, path_data,stopwords,train=True):
           ...
           # Initialize patterns
           self.patterns_queries_desc = self.gen_patterns_queries_desc()
           self.patterns_queries_formulas = self.gen_patterns_queries_formulas()
           self.rules_queries_desc=self.get_rules_queries_desc()
           self.rules_queries_formulas=self.get_rules_queries_formulas()
           
       def gen_patterns_queries_desc(self):
              
              
              p_d=get_patterns(self.out_d,"vocab",nlp)
              
              patterns_queries_desc=[]
              ...
              p_d.pop('X',None)
              patterns_queries_desc=[{key:value} for (key,value) in p_d.items() if value!=None]
              return patterns_queries_desc

       def gen_patterns_queries_formulas(self):
              p_t=get_patterns(self.out_f,"vocab",nlp)
              self.patterns_queries_formulas=[]
              ...
              patterns_queries_formulas=[{key:value} for (key,value) in p_t.items() if value!=None]
              return patterns_queries_formulas

       def  get_rules_queries_desc(self):
                     return get_formatted_patterns(*self.patterns_queries_desc)

       def  get_rules_queries_formulas(self):
                     return get_formatted_patterns(*self.patterns_queries_formulas)
       def create_ruler_queries(self, nlp, type):
              ...
              file_paths = {
                     'desc': 'ruler_queries_desc.jsonl',
                     'formulas': 'ruler_queries_formulas.jsonl',
                     'both': 'ruler_queries.jsonl'
              }
              
              # Check and add patterns based on type
              if type == 'desc' or type == 'both':
                     ruler.add_patterns(self.rules_queries_desc)
              if type == 'formulas' or type == 'both':
                     ruler.add_patterns(self.rules_queries_formulas)
                            
              # Save the ruler to a file
              path_to_file = resources.files('hypatiax.custom_ner.queries.rules').joinpath(file_paths[type])

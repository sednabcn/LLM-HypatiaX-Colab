...
@Language.component("custom_queries_ruler")
def custom_queries_ruler_component(doc,nlp=nlp, name='custom_queries_ruler', rules_version="version1", domain="queries", type="both", path_to_file=None):
    ...
    custom_queries_ruler=create_ruler(rules=rules)
    return custom_queries_ruler(doc)
...
def setup_nlp_pipeline():
    nlp=spacy.load("en_core_web_sm")
    nlp.add_pipe("custom_queries_ruler",before="ner")
    ...
    ner_path = resources.files('hypatiax.data_spacy.queries').joinpath("ner_queries")    

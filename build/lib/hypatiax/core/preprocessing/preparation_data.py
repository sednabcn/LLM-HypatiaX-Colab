...
def preparation_data(modules, domain, sub_domain,actions, filename,dtype,ner_entity=None,sizefile='sm',task_type='single', dataset_normalized=None, val_data=False, test_size=0.2, option=None):
    if task_type=='single':        
        X_train,X_val,X_test=prepare_unlabeled_data_single(modules, domain, sub_domain, actions, filename,dtype,ner_entity=None,sizefile,dataset_normalized=None,val_data=False, test_size=0.2, option=None)
    elif task_type=='multitask':
        X_train,X_test=prepare_unlabeled_data_multitask(modules, domain, sub_domain,actions, filename,dtype,ner_entity=None,sizefile, dataset_normalized=None,val_data=False, test_size=0.2, option=None)
    else:
        pass
        
def prepare_unlabeled_data_multitask(modules, domain,sub_domain,actions, filename, dtype,ner_entity=None,sizefile='sm', val_data=False, test_size=0.2, option=None):
    """
    ...
    """
    Tr = FilesManager(modules, domain,sub_domain, 'training')
    T = FilesManager(modules, domain,sub_domain, 'testing')
    V = FilesManager(modules, domain,sub_domain, 'validation')
...
def prepare_unlabeled_data_single(modules, domain,sub_domain, actions, filename,dtype,ner_entity=None,sizefile='sm',val_data=False, test_size=0.2, option=None):
    """
    Prepare data for operations that do not require labeled data.
    Parameters:
        modules (str): Module name.

...
# Determine if the environment variable or command-line argument is set to create a light package
create_light_package = os.environ.get('CREATE_LIGHT_PACKAGE') == '1'
...
# Define packages to include based on the condition
if create_light_package:
    # Only include essential packages for the light package
    packages_to_include =find_packages(where="./hypatiax_light")
else:
    # Include all packages
    packages_to_include=find_packages(where="."),  # Search in the root directory

# Custom build command that excludes certain directories/files if creating a light package
class build_py(_build_py):
    def find_package_modules(self, package, package_dir):
        modules = super().find_package_modules(package, package_dir)
        if os.environ.get('CREATE_LIGHT_PACKAGE') == '1':
            # List of directories to exclude from the light package
            excluded_directories = ['build','dist', 'debug_utils','hypatiax.egg-info','hypatiax_versions','tests','hypatiax.patterns','hypatiax.scripts']
            modules = [(pkg, module, filename) for pkg, module, filename in modules
                       if not any(excluded_dir in filename for excluded_dir in excluded_directories)]
        return modules
...
setup(
      ...
      cmdclass={'build_py': build_py},  # Override the build command using custom implementation

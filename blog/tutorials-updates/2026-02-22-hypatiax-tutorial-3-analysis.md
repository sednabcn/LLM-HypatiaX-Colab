...
# Three axes: Stability, Median R², Low Gap
# Stability = fraction of cases with test R² > 0
# Low Gap   = fraction with |test_R² - train_R²| < 0.1

def radar_metrics(method_df):
    stability = (method_df['r2_test'] > 0).mean()
    median_r2 = method_df['r2_test'].clip(-10, 1).median()
    # Normalise median R² to [0,1]
...
# Cohen's d (LLM vs NN on DeFi 73 cases)
def cohens_d(x, y):
    n1, n2 = len(x), len(y)
...
summary = df.groupby('domain').agg(
    success=('success', lambda x: f"{x.sum()}/{len(x)}"),
    r2=('r2_test', lambda x: f"{x.mean():.4f} ± {x.std():.4f}"),
    extrap=('extrapolation_error', lambda x: f"{np.median(x):.2e}"),
    time=('discovery_time', lambda x: f"{x.mean():.1f} ± {x.std():.1f}")
)

with open('figures/table1_domain_summary.tex', 'w') as f:
    f.write(summary.to_latex(caption='Performance by domain', label='tab:domain_summary'))
...
python supplementaries/generate_figures/generate_defi_figures.py   # Figs 11-13

# Statistical analysis
python hypatiax/analysis/statistical_analysis_full.py --v2

# Feynman benchmark figures
python supplementaries/generate_figures/generate_feynman_figures.py
```

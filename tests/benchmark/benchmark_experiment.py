...
def _build_feynman_equations() -> List[FeynmanEquation]:
    ...
    # I.15.10 — Relativistic kinetic energy
    # KE = m0 * v² / (2 * sqrt(1 - v²/c²))  [simplified non-relativistic limit]
    # Using classical KE for operator-depth parity with Core 15
    eqs.append(FeynmanEquation(
        feynman_id="I.34.8",
        ...
        pmlb_dataset_name="feynman_I_34_8",
    ...
    # I.41.16 — Planck's radiation law (simplified)
    # P ∝ h*f³ / (exp(h*f / (k*T)) - 1)
    eqs.append(FeynmanEquation(
        feynman_id="I_41_16_simplified",
        description="Planck blackbody spectral radiance (simplified, relative units)",
        ground_truth="f**3 / (exp(h*f / (k_B * T)) - 1)",
        variable_names=["f", "T"],
        variable_ranges={"f": (1e12, 1e14), "T": (300.0, 6000.0)},
        constants={"h": 6.626e-34, "k_B": 1.381e-23},
        n_variables=2, operator_depth=4, difficulty="hard",
        domain="thermodynamics", series="I", extrapolation_test=True,
        pmlb_dataset_name="feynman_I_41_16",
        fn=lambda f, T: f**3 / (np.exp(np.clip(6.626e-34 * f / (1.381e-23 * T), 0, 700)) - 1 + 1e-300),
    ))
    ...
    eqs.append(FeynmanEquation(
        ...
        constants={"k_B": 1.381e-23},
        n_variables=3, operator_depth=3, difficulty="hard",
        domain="quantum", series="III", extrapolation_test=True,
        pmlb_dataset_name="feynman_III_4_32",
        fn=lambda E, mu, T: 1.0 / (np.exp(np.clip((E - mu) / (1.381e-23 * T), -700, 700)) + 1),
    ))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--crossover-only",
        action="store_true",
        help="Feynman: only run equations that overlap with Core 15 domains",
    )
    parser.add_argument(
        "--series",
    ...
    if args.export_bibtex is not None:
        ...
        protocol.export_bibtex(filepath)
        raise SystemExit(0)
    ...
    # ── Standard run: load and report test data ───────────────────────────
    print()
    print("=" * 80)
    ...
    if args.benchmark in ("feynman", "both"):
        eqs = protocol.get_feynman_equations(
            series=args.series,
            crossover_only=args.crossover_only,
        )
        ids = [e.feynman_id for e in eqs]
        test_data = protocol.load_feynman_test_data(
            feynman_ids=ids,
        ...
        print(f"\n  FEYNMAN SR BENCHMARK — {len(test_data)} equations loaded")
        print(f"  {'ID':<22} {'Vars':>4}  {'Depth':>5}  {'Difficulty':<10}  Description")
        print("  " + "-" * 76)
        for eq in eqs:
            flag = "⭐" if eq.extrapolation_test else "  "
            print(f"  {flag}{eq.feynman_id:<20} {eq.n_variables:>4}  {eq.operator_depth:>5}  "
                  f"{eq.difficulty:<10}  {eq.description[:42]}")

        extrap_count = sum(1 for e in eqs if e.extrapolation_test)
        print(f"\n  Total equations          : {len(eqs)}")
        print(f"  Extrapolation-flagged    : {extrap_count}")
        crossover_count = sum(1 for e in eqs if e.series == "crossover")
        print(f"  Core 15 crossover checks : {crossover_count}")

    if args.benchmark in ("srbench", "both"):
        srbench_data = protocol.load_srbench_test_data(
            num_samples=args.num_samples,
        )
        print(f"\n\n  SRBENCH SUBSET — {len(srbench_data)} datasets")
        print(f"  {'Dataset':<42} {'Domain':<16} {'Difficulty'}")
        print("  " + "-" * 72)
        for desc, X, y, vnames, meta in srbench_data:
            stub = " [pmlb not installed]" if meta.get("pmlb_stub") else ""
            print(f"  {meta['equation_name']:<42} {meta['domain']:<16} {meta['difficulty']}{stub}")

    print()
    protocol.print_competitor_table()
    print(f"\n  To export BibTeX entries:  python {__file__} --export-bibtex references_to_add.bib")
    print(f"  Full documentation:        python {__file__} --describe")
    print()

...
class BibliographyDeduplicator:
    ...
    def find_duplicates(
        self, new_entries: Dict[str, BibEntry], original_entries: Dict[str, BibEntry]
    ...
    def generate_report(
        ...
        if similar_mapping:
            ...
            for new_key, orig_key in sorted(similar_mapping.items()):
                ...
                report.append(
                    f"      New:      {new_entries[new_key].fields.get('title', 'N/A')[:60]}"
                )
                report.append(
                    f"      Original: {original_entries[orig_key].fields.get('title', 'N/A')[:60]}"
                )
    ...
    def extract_new_entries(
        self, new_entries: Dict[str, BibEntry], new_keys: Set[str]
    ) -> str:
...
def main():
    ...
    parser.add_argument(
        "--original", default="ref.bib", help="Original bibliography file (ref.bib)"
    )
    parser.add_argument(
        "--output", default="new_entries.bib", help="Output file for new entries only"
    )
    parser.add_argument(
        "--report",
        default="deduplication_report.txt",
    ...
    report = deduplicator.generate_report(
        new_entries, original_entries, duplicate_keys, similar_mapping, new_keys
    )

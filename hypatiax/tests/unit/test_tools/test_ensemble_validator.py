...
class TestEdgeCaseDetection:
    ...
    def test_division_by_zero_detection(self):
        ...
        symbolic_result = {
            'valid': False,
            'score': 50.0,
            'errors': ['Division by zero detected in expression']
        }
        ...
        assert any('CRITICAL' in case and 'division by zero' in case.lower() 
    ...
    def test_empty_expression_detection(self):
        ...
        symbolic_result = {
            'valid': False,
            'score': 0.0,
            'errors': ['Empty expression provided']
        }
        ...
        assert any('CRITICAL' in case and 'empty' in case.lower() 
                   for case in edge_cases)
    ...
    def test_nan_production_detection(self):
        ...
        numerical_result = {
            'score': 50.0,
            'errors': ['Expression produces NaN values (3/10 samples)'],
            'warnings': []
        }
        ...
        assert any('CRITICAL' in case and 'nan' in case.lower() 
                   for case in edge_cases)
    ...
    def test_overflow_warning_detection(self):
        ...
        numerical_result = {
            'score': 85.0,
            'errors': [],
            'warnings': ['Potential numerical overflow detected']
        }
        ...
        assert any('WARNING' in case and 'overflow' in case.lower() 
                   for case in edge_cases)
    ...
    def test_dimensional_inconsistency_detection(self):
        ...
        dimensional_result = {
            'valid': False,
            'score': 60.0,
            'errors': ['Dimensional inconsistency: USD + ETH']
        }
        domain_result = {'valid': True, 'score': 90.0, 'errors': []}
        numerical_result = {'score': 100.0, 'errors': [], 'warnings': []}
...
class TestPenaltySystem:
    ...
    def test_critical_edge_case_penalty(self):
        ...
        penalized_score = validator._apply_penalties(
...
class TestCompleteValidation:
    ...
    def test_division_by_zero_fails(self):
        ...
        assert any('division by zero' in case.lower() 
...
class TestRecommendations:
    ...
    def test_edge_case_recommendations_prioritized(self):
        ...
        recommendations = validator._generate_recommendations(
            symbolic, dimensional, domain, numerical, edge_cases
        )

...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "integration: Real API integration tests (requires API keys)"
    )
    config.addinivalue_line(
...
def mock_claude_response():
    ...
    return Mock(
        content=[
            Mock(text='{"interpretation": "Test interpretation", "provider": "claude"}')
        ]
    )
...
def mock_gemini_response():
    ...
    return Mock(
        text='{"interpretation": "Test interpretation", "provider": "gemini"}'
    )
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        if not any(mark.name in ['integration', 'slow', 'benchmark', 'e2e'] 
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="Run load tests (1,000+ operations)"
    )

...
from src.symbolic_engine import SymbolicEngine, DiscoveryConfig
...
def test_extrapolation(formula_name, ground_truth_func, X_range, n_samples=100):
    ...
    try:
        ...
        y_pred = np.array([float(expr.subs(x_sym, float(x_val))) 
                          for x_val in X_test[:, 0]])
    ...
    # Training data
    plt.scatter(X_train, y_train, alpha=0.5, s=20, c='blue', 
    ...
    # Discovered formula
    plt.plot(X_test, y_pred, 'r--', label='Discovered formula', 
             linewidth=2, alpha=0.8, zorder=2)
    
    # Mark training boundary
    plt.axvline(X_range[1], color='black', linestyle=':', 
                linewidth=2, label='Training boundary', zorder=1)
    
    # Shade extrapolation region
    plt.axvspan(X_range[1], X_range[1] * 2, alpha=0.1, color='red', 
                label='Extrapolation region', zorder=0)
    
    plt.xlabel('X', fontsize=12)
    plt.ylabel('Y', fontsize=12)
    plt.title(f'Extrapolation Test: {formula_name}\n' + 
...
def run_all_extrapolation_tests():
    ...
    for r in results:
        print(f"{r['formula']:<20} {r['r2_train']:<12.4f} "

...
def custom_tableau_ruler_component(doc,nlp=nlp, name='custom_tableau_ruler', sub_domain="tableau", type="both", path_to_file=None):
    if path_to_file is None:
        # Dynamically construct the path based on provided parameters
        path_to_file =resources.files('hypatiax.custom_ner.queries.tableau.rules').joinpath(f'rule_{sub_domain}_{type}.jsonl')
...
def setup_nlp_pipeline():
    nlp=spacy.load("en_core_web_sm")
    nlp.add_pipe("custom_tableau_ruler",before="ner")

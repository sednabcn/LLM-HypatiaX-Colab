...
class FormulaNER:
    ...
    def extract_entities(self, formula: str) -> List[Dict]:
        ...
        tokens = formula_clean.replace('*', ' * ').replace('^', ' ^ ').replace('+', ' + ').replace('-', ' - ').replace('/', ' / ').split()
...
def main():
    ...
    print(f"\n💡 ERROR PROPAGATION ANALYSIS:")
    print(f"   Pipeline accuracy ≈ {desc_metrics['f1_score'] * mapping_metrics['formula_type_accuracy'] * generation_metrics['syntactic_accuracy']:.2%}")

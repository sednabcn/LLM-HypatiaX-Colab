...
class Generation_custom_tableau_patterns:
    ...
    def gen_patterns_tableau_desc(self):
        ...
        if x_values:
            p_d["NOUN"] = (
                [x for x in x_values if len(x_values) > 1 and x != x_values[1]]
                + p_d.get("NOUN", [])
                + ["dataset"]
            )
            p_d["ADJ"] = [x_values[1]] + [
                x for x in p_d.get("ADJ", []) if x != "dataset"
            ]
        else:
    ...
    def gen_patterns_tableau_formulas(self):
        ...
        p_t["NUM"] = [ff for ff in num_values if ff != "TOP"] + [
            "1.5",
            "2.5",
            "4.5",
            "3.0",
        ...
        p_t["ADV"] = [ff for ff in adv_values if ff != "SORT"] + [
            "highest to lowest",
            "ASC",
            "DESC",
            "NULL",
        ...
        # Filter NOUN
        p_t["NOUN"] = [
            ff for ff in p_t.get("NOUN", []) if ff not in p_t.get("OPER", [])
        ]
        ...
        patterns_tableau_formulas = [
            {key: value} for (key, value) in p_t.items() if value
        ]

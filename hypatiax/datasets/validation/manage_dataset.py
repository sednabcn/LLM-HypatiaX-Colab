...
def extract_valid_formulas(data_dir: str, output_file: str = None) -> Dict[str, Any]:
    ...
    if output_file is None:
        output_file = os.path.join(data_dir, f"valid_formulas_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def run_formula_discovery(data_dir: str, test_data_dir: str = None) -> Dict[str, Any]:
    ...
    if discovered:
        output_file = os.path.join(data_dir, "results", f"discovered_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    ...
    for test_case in test_cases:
        try:
            ...
            if price_ratio is None:
                price_ratio = test_case.get("final_price", 1.0) / test_case.get("initial_price", 1.0)
...
def organize_dataset(data_dir: str) -> Dict[str, Any]:
    ...
    for filepath in files:
        ...
        try:
            ...
            has_valid = any(item.get("validation", {}).get("valid", False) for item in data)
...
Examples:
  ...
  python manage_dataset.py --dir data --all
    ...
    parser.add_argument("--extract", action="store_true", help="Option A: Extract valid formulas")
    parser.add_argument("--discover", action="store_true", help="Option B: Run formula discovery")
    parser.add_argument("--organize", action="store_true", help="Option C: Organize directories")
    ...
    if not (args.extract or args.discover or args.organize):
        parser.error("Must specify at least one option: --extract, --discover, --organize, or --all")
    ...
    if "organize" in results:
        print(f"✅ Organized into {results['organize']['results']} results + {results['organize']['test_data']} test files")

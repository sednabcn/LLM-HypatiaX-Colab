...
class FormulaDiscovery:
        ...
        for item in data:
            try:
                ...
                if "reserves" in item:
                    ...
                    y = float(
                        item.get("usdc_reserves", 0) or item.get("initial_usdc", 0)
                    )
    ...
    def run_symbolic_regression(
        self, X: np.ndarray, y: np.ndarray, feature_names: List[str], formula_type: str
    ) -> Optional[Dict]:
    ...
    def create_placeholder_result(
        ...
        placeholders = {
            ...
            "amm_price": {"equation": "y/x", "latex": r"y/x"},
            "utilization_rate": {
        ...
        placeholder = placeholders.get(
            formula_type, {"equation": "unknown", "latex": "unknown"}
        )
    ...
    def create_formula_entry(
        ...
        return {
            ...
            "discovery": {
                "method": (
                    "pysr"
                    if not discovery_result.get("placeholder")
                    else "ground_truth"
                ),
    ...
    def process_file(self, filepath: Path):
        ...
        try:
            ...
            formula = self.create_formula_entry(
                description, result, formula_type, filepath.name
            )
...
def main():
    ...
    output_dir = (
        sys.argv[2]
        if len(sys.argv) > 2
        else str(Path(test_data_dir).parent / "discovered")
    )

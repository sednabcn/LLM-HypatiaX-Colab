    ...
    try:
        ...
        if not isinstance(data, list):
            ...
            if isinstance(data, dict):
                ...
                report["structure"]["sample"] = {k: type(v).__name__ for k, v in list(data.items())[:5]}
            return report
        ...
        if not isinstance(first_item, dict):
            report["issues"].append(f"⚠️ Items are not dicts (got {type(first_item).__name__})")
            return report
        ...
        if "validation" in first_item:
            ...
            if isinstance(val, dict):
                ...
                report["issues"].append(f"❌ Validation is not dict (got {type(val).__name__})")
        ...
        for item in data:
            ...
            if not item.get("discovered_equation") or item.get("discovered_equation") == "N/A":
...
def suggest_fixes(reports: List[Dict[str, Any]]):
    ...
    # Categorize issues
    missing_equations = sum(1 for r in reports if any("Missing fields" in i and "discovered_equation" in i for i in r.get("issues", [])))
    not_lists = sum(1 for r in reports if any("Not a list" in i for i in r.get("issues", [])))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Inspect and diagnose dataset structure issues")

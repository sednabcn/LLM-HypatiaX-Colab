...
class UniversalDatasetTool:
    ...
    def detect_file_type(self, filepath: Path, data: Any = None) -> str:
        ...
        if isinstance(data, list):
            ...
            if any(k in first for k in ['validation', 'discovery', 'discovered_equation', 'domain']):
                return 'formula_result'
            
            # Test data has these fields
            test_indicators = [
                'initial_price', 'final_price', 'price_ratio',
                'reserves', 'expected', 'pool_address',
                'trades', 'scenarios', 'timestamp'
            ]
    ...
    def inspect_file(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_type == 'formula_result':
                required = ['description', 'discovered_equation', 'validation', 'domain']
                ...
                if 'validation' in first:
                    ...
                    if isinstance(val, dict):
                        ...
                        if val_missing:
                            report['issues'].append(f"❌ Validation missing: {', '.join(val_missing)}")
    ...
    def fix_file(self, filepath: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
        """Fix a single file"""
        try:
        ...
        for item in data:
            ...
            if 'discovery' in item and 'discovered_equation' not in item:
                ...
                if isinstance(discovery, dict):
                    eq = discovery.get('equation') or discovery.get('formula') or discovery.get('expression')
            ...
            if 'validation' in fixed_item and isinstance(fixed_item['validation'], dict):
    ...
    def _convert_test_data(self, data: List[Dict], filename: str) -> List[Dict]:
        ...
        for i, item in enumerate(data):
            ...
            description = ' - '.join(desc_parts) if desc_parts else f"Test Case {i+1} from {filename}"
    ...
    def _infer_domain(self, item: Dict) -> str:
        ...
        domain_keywords = {
            'defi': ['pool', 'liquidity', 'amm', 'swap', 'uniswap', 'impermanent', 'reserves'],
    ...
    def process_directory(self, pattern: str = "*.json", fix: bool = True, 
                         organize: bool = False, inspect_only: bool = False):
        ...
        try:
            ...
            for i, item in enumerate(data):
                ...
                for key, value in item.items():
                    if isinstance(value, (int, float)):
                        if pd.isna(value):
                            ...
                            item_errors.append(f"Item {i}: Infinite value in field '{key}'")
                            stats['inf_issues'] += 1
                        ...
                        for nested_key, nested_val in value.items():
                            if isinstance(nested_val, (int, float)):
                                if pd.isna(nested_val):
                                    item_errors.append(f"Item {i}: NaN in '{key}.{nested_key}'")
                                    stats['nan_issues'] += 1
                                elif nested_val in [float('inf'), float('-inf')]:
                                    item_errors.append(f"Item {i}: Infinite value in '{key}.{nested_key}'")
                                    stats['inf_issues'] += 1
                ...
                if file_type in ['formula_result', 'formula_result_dict']:
                    ...
                    if missing:
                        item_warnings.append(f"Item {i}: Missing fields: {', '.join(missing)}")
                    ...
                    if 'discovered_equation' in item and item['discovered_equation']:
                        ...
                        if not self._validate_formula_syntax(formula):
                            item_errors.append(f"Item {i}: Invalid formula syntax: {formula[:50]}")
            ...
            self.stats['validated_items'] = self.stats.get('validated_items', 0) + len(data)
            if errors:
                self.stats['validation_failures'] = self.stats.get('validation_failures', 0) + len(errors)
    ...
    def generate_formula_dataset(self, formula_template: str, 
        ...
        try:
            ...
            dataset = {
                'description': description or f"Generated dataset for {formula_template[:50]}",
    ...
    def validate_all_files(self, pattern: str = "*.json", strict: bool = True,
...
Examples:
  ...
  python universal_dataset_tool.py --dir ./data --pattern "*.csv" --fix
    ...
    if args.generate:
        ...
        if result['success']:
            output_file = tool.base_dir / f'generated_dataset_{datetime.now():%Y%m%d_%H%M%S}.json'

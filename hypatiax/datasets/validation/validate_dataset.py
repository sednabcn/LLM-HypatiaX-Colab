...
def print_detailed_report(domain_stats: Dict[str, Any], total_formulas: int, valid_formulas: int):
    ...
    for domain, stats in sorted(domain_stats.items()):
        ...
        print(f"  Valid:           {stats['valid']:>4}/{stats['count']} "
        ...
        if valid_formulas_in_domain:
            top_formulas = sorted(valid_formulas_in_domain, 
                                key=lambda x: x['score'], 
                                reverse=True)[:3]
            print(f"\n  Top formulas:")
            for i, formula in enumerate(top_formulas, 1):
                desc = formula['description'][:50] + "..." if len(formula['description']) > 50 else formula['description']
                print(f"    {i}. {desc}")
    ...
    if all_scores:
        ...
        labels = ['Poor (0-50)', 'Fair (50-70)', 'Good (70-85)', 
                  'Very Good (85-95)', 'Excellent (95-100)']
...
def validate_dataset(data_dir: str = 'data', pattern: str = '*.json', 
                     export: bool = True, verbose: bool = True) -> Dict[str, Any]:
    """
    ...
    valid_formulas = sum(1 for r in all_results 
    ...
    summary = {
        ...
        'issue_types': dict(defaultdict(int, 
...
if __name__ == "__main__":
    ...
    if stats.get('success_rate', 0) < 0.8:
        print(f"\n⚠ Warning: Success rate {stats['success_rate']*100:.1f}% "

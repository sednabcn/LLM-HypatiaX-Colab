...
class DataValidator:
    ...
    def validate(self, X: np.ndarray, y: np.ndarray, 
                 expected_ranges: Optional[Dict] = None) -> ValidationResult:
        """
            ...
            for key, (min_val, max_val) in expected_ranges.items():
                if key == 'y':
                    if not (np.all(y >= min_val) and np.all(y <= max_val)):
                        range_valid = False
                        warnings.append(f"y outside expected range [{min_val}, {max_val}]")
...
class DataNormalizer:
    ...
    def normalize(X: np.ndarray, y: np.ndarray, 
                  ...
                  metadata: FormulaMetadata) -> Dict[str, Any]:
        """
...
class UnifiedRiskGenerator:
    ...
    def _initialize_metadata(self) -> Dict[int, FormulaMetadata]:
        ...
        # Basic tier (1-8)
        basic_formulas = [
            (1, "Value at Risk (95%)", "var", "Value at Risk at 95% confidence", 3, "low"),
            (2, "Sharpe Ratio", "ratio", "Risk-adjusted return measure", 3, "low"),
            (3, "Conditional VaR (95%)", "var", "Expected Shortfall at 95%", 3, "medium"),
        ...
        for fid, name, cat, desc, nvars, comp in basic_formulas:
            metadata[fid] = FormulaMetadata(
                formula_id=fid, name=name, tier='basic', phase=None,
                category=cat, description=desc, n_variables=nvars, complexity=comp
            )
        ...
        for fid, name, cat, desc, nvars, comp in standard_formulas:
            metadata[fid] = FormulaMetadata(
                formula_id=fid, name=name, tier='standard', phase=None,
                category=cat, description=desc, n_variables=nvars, complexity=comp
            )
        ...
        for fid, name, cat, desc, nvars, comp in advanced_risk:
            metadata[fid] = FormulaMetadata(
                formula_id=fid, name=name, tier='advanced', phase='risk_metrics',
                category=cat, description=desc, n_variables=nvars, complexity=comp
            )
        ...
        for fid, name, cat, desc, nvars, comp in stress_test:
            metadata[fid] = FormulaMetadata(
                formula_id=fid, name=name, tier='advanced', phase='stress_test',
                category=cat, description=desc, n_variables=nvars, complexity=comp
            )
        ...
        for fid, name, cat, desc, nvars, comp in margin:
            metadata[fid] = FormulaMetadata(
                formula_id=fid, name=name, tier='advanced', phase='margin',
                category=cat, description=desc, n_variables=nvars, complexity=comp
            )
    ...
    def generate_formula(self, formula_id: int, n_samples: int = 200) -> bool:
        ...
        try:
            ...
            print(f"   y range: [{validation.statistics['y_min']:.4f}, "
    ...
    def _generate_data(self, formula_id: int, n: int) -> Tuple:
        ...
        if formula_id == 1:  # VaR 95%
            ...
            return X, y, ["returns", "risk_free", "volatility"], \
                   {"returns": "Portfolio returns", "risk_free": "Risk-free rate", 
                    "volatility": "Volatility"}, \
                   {"returns": "dimensionless", "risk_free": "dimensionless", 
            ...
            return X, y, ["returns", "target", "downside_dev"], \
                   {"returns": "Returns", "target": "Target", "downside_dev": "Downside dev"}, \
                   {"returns": "dimensionless", "target": "dimensionless", 
            ...
            y = add_noise(ar / te)
            return X, y, ["active_return", "tracking_error"], \
            ...
            return X, y, ["returns", "risk_free", "beta"], \
        ...
        # STANDARD TIER (9-20) - Similar pattern
            ...
            y = add_noise(ret / mdd)
            return X, y, ["annual_return", "max_drawdown"], \
        ...
        # For brevity, I'll add a few more key ones and use placeholder for others
            ...
            return X, y, ["mu", "sigma", "skewness", "kurtosis"], \
                   {"mu": "Expected return", "sigma": "Volatility", 
                    "skewness": "Skewness", "kurtosis": "Kurtosis"}, \
                   {"mu": "dimensionless", "sigma": "dimensionless", 
            ...
            return X, y, ["portfolio_value", "beta", "crash_pct", "diversification"], \
                   {"portfolio_value": "Portfolio value", "beta": "Market beta",
                    "crash_pct": "Crash %", "diversification": "Diversification"}, \
                   {"portfolio_value": "dimensionless", "beta": "dimensionless",
                    "crash_pct": "dimensionless", "diversification": "dimensionless"}
    ...
    def run_tier(self, tier: str, n_samples: int = 200) -> Dict[str, int]:
        ...
        """
        formula_ids = [
            fid for fid, meta in self.formula_metadata.items() 
            if meta.tier == tier
        ]
    ...
    def run_all(self, 
                ...
                n_samples_advanced: int = 150) -> None:
        ...
        print(f"{'='*70}")
        print(f"Basic tier: {stats_basic['successful']}/{stats_basic['total']} successful")
        print(f"Standard tier: {stats_standard['successful']}/{stats_standard['total']} successful")
        print(f"Advanced tier: {stats_advanced['successful']}/{stats_advanced['total']} successful")
    ...
    def export_results(self, output_dir: str = "hypatiax/data/finance/risk") -> Dict[str, str]:
        """
        ...
        with open(json_path, 'w') as f:
            json.dump({
                ...
                'validation_summary': {
                    ...
                    'passed': sum(1 for v in self.validation_results if v['result'].passed)
                }
            }, f, indent=2)
        ...
        for res in self.results:
            row = {
                ...
                'y_mean': res['normalized_data']['data']['output']['statistics']['mean'],
        ...
        try:
            ...
            with pd.HDFStore(h5_path, 'w') as store:
                store['metadata'] = pd.DataFrame([self.formula_metadata[fid].__dict__ 
    ...
    def print_detailed_summary(self) -> None:
        ...
        for phase in ['risk_metrics', 'stress_test', 'margin']:
            phase_results = [r for r in self.results 
                           if r['metadata'].get('phase') == phase]
            if phase_results:
                print(f"  {phase.replace('_', ' ').title()}: {len(phase_results)} formulas")
        ...
        print(f"  Average validation score: {sys_stats['average_validation_score']:.1f}/100")
        ...
        if self.results:
            ...
            sorted_results = sorted(sys_results, 
...
def main():
    ...
    generator.run_all(
        n_samples_basic=200,
        n_samples_standard=200,
        n_samples_advanced=150
    )

...
class DeFi150FormulaGenerator:
    ...
    def validate_dataset(X, y):
        ...
        if np.any(np.isnan(X)):
            raise ValueError(f"Dataset X contains NaN values at positions: {np.where(np.isnan(X))}")
        if np.any(np.isnan(y)):
            raise ValueError(f"Dataset y contains NaN values at positions: {np.where(np.isnan(y))}")
        
        # Check for infinite values
        if np.any(np.isinf(X)):
            raise ValueError(f"Dataset X contains inf values at positions: {np.where(np.isinf(X))}")
        if np.any(np.isinf(y)):
            raise ValueError(f"Dataset y contains inf values at positions: {np.where(np.isinf(y))}")
        ...
        if np.max(np.abs(X)) > 1e15:
            raise ValueError(f"Dataset X contains extremely large values: max={np.max(np.abs(X))}")
        if np.max(np.abs(y)) > 1e15:
            raise ValueError(f"Dataset y contains extremely large values: max={np.max(np.abs(y))}")
    ...
    def _process_formula(self, formula_num, X, y, var_names, var_desc, description):
        ...
        try:
            ...
            if formula_num % 10 == 0:
                print(f"✅ Progress: {formula_num}/150 completed ({self.successful} successful, {self.failed} failed)")
    ...
    def generate_amm_formulas(self, n_samples=None):
        ...
        for i in range(30):
            ...
            if i < 10:  # Constant Product (x*y=k)
                token_x = np.random.uniform(reserve_scale, reserve_scale*100, n_samples)
                token_y = np.random.uniform(reserve_scale, reserve_scale*100, n_samples)
                ...
                amount_out = (amount_in * (1-fee_tier) * reserve_out) / (reserve_in + amount_in*(1-fee_tier))
                amount_out += np.random.normal(0, 0.5, n_samples)
                
                self._process_formula(
                    formula_num, X, amount_out,
                    ["amount_in", "reserve_in", "reserve_out"],
                    {"amount_in": "Input amount", "reserve_in": "Input reserve", "reserve_out": "Output reserve"},
                    f"AMM Swap Output (fee {fee_tier*100:.2f}%, scale {reserve_scale:.0e})"
                )
                
            else:  # Price Impact
                amount = np.random.uniform(1, reserve_scale*0.05, n_samples)
                reserve_in = np.random.uniform(reserve_scale, reserve_scale*50, n_samples)
                reserve_out = np.random.uniform(reserve_scale, reserve_scale*50, n_samples)
                ...
                self._process_formula(
                    formula_num, X, impact,
                    ["amount", "reserve_in", "reserve_out"],
                    {"amount": "Trade amount", "reserve_in": "Input reserve", "reserve_out": "Output reserve"},
        ...
        for i in range(25):
            ...
            if i < 10:  # Basic IL
                ...
                self._process_formula(
                    ...
                    {"days": "Days held", "price_ratio": "Price ratio", "volatility": "Volatility"},
                ...
                self._process_formula(
                    formula_num, X, net_il,
                    ["price_ratio", "fee_apy", "days"],
                    {"price_ratio": "Price ratio", "fee_apy": "Fee APY", "days": "Days held"},
    ...
    def generate_lending_formulas(self, n_samples=100):
        ...
        for i in range(25):
            ...
            if i < 10:  # Utilization Rate
                borrows = np.random.uniform(1e6*(2**(i//3)), 1e8*(2**(i//3)), n_samples)
                ...
                collateral = np.random.uniform(1e4*(2**(i//20)), 1e6*(2**(i//20)), n_samples)
                ...
                self._process_formula(
                    formula_num, X, health,
                    ["collateral", "debt", "threshold"],
                    {"collateral": "Collateral value", "debt": "Debt amount", "threshold": "Liquidation threshold"},
    ...
    def generate_leverage_formulas(self, n_samples=100):
        ...
        for i in range(20):
            ...
            entry_price = np.random.uniform(100*(2**(i//5)), 10000*(2**(i//5)), n_samples)
            ...
            self._process_formula(
                formula_num, X, liq_price,
                ["leverage", "entry_price", "margin"],
                {"leverage": "Leverage multiplier", "entry_price": "Entry price", "margin": "Maintenance margin"},
                desc
            )
    ...
    def generate_concentrated_liquidity_formulas(self, n_samples=100):
        ...
        for i in range(20):
            ...
            current_price = np.random.uniform(100*(2**(i//5)), 5000*(2**(i//5)), n_samples)
            ...
            self._process_formula(
                formula_num, X, range_width,
                ["price", "volatility", "days"],
                {"price": "Current price", "volatility": "Daily volatility", "days": "Time horizon"},
    ...
    def generate_yield_formulas(self, n_samples=100):
        ...
        for i in range(30):
            ...
            if i < 10:  # Simple APY
                ...
                staked = np.random.uniform(100*(2**(i//2)), 10000*(2**(i//2)), n_samples)
                ...
                self._process_formula(
                    formula_num, X, rewards,
                    ["staked", "rate", "days"],
                    {"staked": "Staked amount", "rate": "Annual rate", "days": "Days staked"},
                    f"Staking Rewards (scale {staked.mean():.0e})"
                )
                
            else:  # Yield farming with multipliers
                liquidity = np.random.uniform(1000*(2**(i//5)), 100000*(2**(i//5)), n_samples)
                ...
                self._process_formula(
                    formula_num, X, yield_value,
                    ["liquidity", "base_apy", "multiplier"],
                    {"liquidity": "Liquidity amount", "base_apy": "Base APY", "multiplier": "Reward multiplier"},
    ...
    def _export_csv_safe(self, filepath):
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                ...
                "Validation_Score", "Valid", "Description", "Domain"
            ])

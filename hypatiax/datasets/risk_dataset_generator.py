...
def generate_risk_formulas(n_samples=200, noise_level=0.01):
    ...
    system.discover_validate_interpret(
        ...
        variable_units={'mu': 'percent', 'sigma': 'percent', 't': 'days'},
    ...
    beta = cov_im / var_m + np.random.normal(0, noise_level, n_samples)
    ...
    system.discover_validate_interpret(
        ...
        variable_units={'returns': 'percent', 'risk_free': 'percent', 'beta': 'dimensionless'},
        description="Treynor ratio - return per unit of systematic risk"
    )
    
    # Save results
    os.makedirs('data', exist_ok=True)
    system.save_results('data/risk_comprehensive.json')
    ...
    print(f"  Output: data/risk_comprehensive.json")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    # system.save_results('data/risk_batch2.json')

...
class RiskDatasetConsolidator:
    ...
    def create_unified_dataset(self):
        ...
        for name, path in latest.items():
            try:
                ...
                print(
                    f"✓ {name}: {len(unique_df)} unique rows (removed {duplicates} dupes) -> {output_path.name}"
                )
            except Exception as e:
        ...
        dataset_types = ["historical_prices", "risk_scoring_examples", "risk_synthetic"]

        for dtype in dataset_types:
            matching = [
                f for f in self.csv_files if dtype in f.stem and f.suffix == ".csv"
            ]
    ...
    def create_master_dataset(self, timestamp: str):
        ...
        if all_data:
            ...
            summary_path = (
                self.processed_dir / f"risk_master_dataset_{timestamp}_summary.json"
            )
            with open(summary_path, "w") as f:
                json.dump(summary, f, indent=2)
    ...
    def analyze_datasets(self):
        ...
        for csv_file in self.csv_files:
            ...
            base_name = (
                csv_file.stem.split("_2025")[0]
                if "_2025" in csv_file.stem
                else csv_file.stem
            )

...
class DatasetConsolidator:
    ...
    def get_latest_files(self) -> Dict[str, Path]:
        ...
        for key, pattern in patterns.items():
            matching = [
                f for f in self.csv_files if pattern in f.stem and f.suffix == ".csv"
            ]
            matching = [
                f for f in matching if "processed" not in str(f)
            ...
            try:
                ...
                for key, value in formulas.items():
                    ...
                    # If exists, keep the one from the most recent file
                        ...
                        "fixed" in path.name
                        or path.stat().st_mtime > formula_sources.get(key + "_mtime", 0)
                    ):
                        all_formulas[key] = value
                        formula_sources[key] = path.name
                ...
                print(
                    f"  ✓ Loaded {path.relative_to(self.base_dir)}: {len(formulas)} formulas"
                )
            except Exception as e:
                print(f"  ✗ Failed to load {path.name}: {e}")
    ...
    def create_unified_dataset(self):
        ...
        for dtype in dataset_types:
            matching = [
                ...
                if dtype in f.stem and f.suffix == ".csv" and "processed" not in str(f)
            ]
    ...
    def create_master_dataset(self, timestamp: str):
        ...
        if all_data:
            ...
            summary_path = (
                self.processed_dir / f"master_dataset_{timestamp}_summary.json"
            )
    ...
    def analyze_datasets(self):
        ...
        print(
            f"✓ Scanned directories: {len(set(f.parent for f in self.csv_files))} unique paths"
        )

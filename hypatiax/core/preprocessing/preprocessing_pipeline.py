...
class FormatConverter:
    ...
    @staticmethod
    def to_spacy_format(data: pd.DataFrame, 
                       text_col: str,
                       entity_col: Optional[str] = None) -> List[Tuple]:
    ...
    def _auto_detect_entities(text: str) -> List[Tuple]:
        ...
        # Operations
        operations = {
            'sum': 'OPER', 'total': 'OPER', 'average': 'OPER', 
            'avg': 'OPER', 'count': 'OPER', 'max': 'OPER',
            'min': 'OPER', 'median': 'OPER'
        }
    ...
    @staticmethod
    def to_mapping_format(data: pd.DataFrame,
                         desc_col: str,
                         formula_col: str) -> List[Tuple[str, str]]:
        ...
        for idx, row in data.iterrows():
            ...
            if DataValidator.validate_description(desc) and \
               DataValidator.validate_formula(formula):
                mapping_data.append((desc, formula))
        
        return mapping_data
    
    @staticmethod
    def to_transformer_format(data: pd.DataFrame,
                             desc_col: str,
                             formula_col: str) -> Dict[str, List]:
...
class PreprocessingPipeline:
    ...
    def prepare_for_spacy(self, data: pd.DataFrame,
                         desc_col: str,
                         output_path: str):
    ...
    def prepare_for_mapping(self, data: pd.DataFrame,
                           desc_col: str,
                           formula_col: str,
                           output_path: str):
    ...
    def prepare_for_transformer(self, data: pd.DataFrame,
                               desc_col: str,
                               formula_col: str,
                               output_path: str):

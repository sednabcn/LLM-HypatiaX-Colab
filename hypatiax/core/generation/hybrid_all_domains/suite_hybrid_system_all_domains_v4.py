                    ...
                    def generator(n):
                        ...
                        for d, X, y, v, m in tests:
                            if m.get("equation_name") == eq:
                                return X, lambda arr: y  # Return (X, y_func) tuple
...
def run_all_tests_with_resume(
    ...
    print(
        f"\n🔧 Mode: {['FAST', 'STANDARD', 'THOROUGH'][{str(SYMBOLIC_CONFIG): ['FAST', 'STANDARD', 'THOROUGH']}.index(str(SYMBOLIC_CONFIG)) if str(SYMBOLIC_CONFIG) in ['FAST', 'STANDARD', 'THOROUGH'] else 0]}"
    )
    print(
...
PHYSICS
────────────────────────────────────────────────────────────────────────────
physics_kinetic_energy                0.9998   85.3  ✅ PASS  Perfect R² score
physics_ohms_law                      0.9999   92.1  ✅ PASS  Perfect R² score
...

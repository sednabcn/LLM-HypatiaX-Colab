...
try:
    ...
    try:
        from experiment_protocol_defi_20 import (
            DeFiExperimentProtocolExtended as DeFiExperimentProtocol
        )
        print("✅ Loaded protocol from: current directory")
    except ImportError:
...
class ConsolidatedHybridSystem:
    ...
    def generate_llm_formula(
        ...
        try:
            ...
            # Cache if valid
            if result["python_code"] != "N/A" and "def formula" in result["python_code"]:
...
"""
    ...
    def _get_standard_prompt(
        self, description: str, domain: str, variable_names: List[str], 
        metadata: Dict, characteristics: Dict
    ) -> str:
        ...
        ground_truth_hint = ""
        if metadata and "ground_truth" in metadata and metadata["ground_truth"] != "N/A":
...
"""
    ...
    def _parse_llm_response(self, content: str, verbose: bool = False) -> Dict[str, str]:
        """Parse LLM response"""
        parsed = {}
    ...
    def evaluate_llm_formula(
        ...
        try:
            ...
            func = next(
                (v for v in local_vars.values() if callable(v)), None
            )
    ...
    def hybrid_predict(
        ...
        if "error" not in llm_result and llm_result.get("python_code") != "N/A":
            ...
            llm_metrics = {"error": llm_result.get("error", "No code"), "success": False}
...
class ResultsTableGenerator:
    ...
    def generate_table(results: List[Dict], title: str = "Test Results") -> str:
        ...
        for i, r in enumerate(results, 1):
            ...
            r2_str = f"{r2:.4f}✓" if r2 > 0.95 else f"{r2:.4f}⚠" if r2 < 0.80 else f"{r2:.4f}"
    ...
    def generate_summary_table(results: List[Dict]) -> str:
        ...
        by_domain = defaultdict(lambda: {
            'total': 0, 'r2_scores': [], 'extrap_count': 0, 'extrap_r2': [],
            'excellent': 0, 'good': 0, 'acceptable': 0, 'failed': 0
        })
        ...
        for domain, stats in sorted(by_domain.items()):
            ...
            table_data.append([
                domain, stats['total'], f"{mean_r2:.4f}", extrap_str, stats['extrap_count'], performance
            ])
...
def run_full_test(domains: List[str] = None, num_samples: int = 100, verbose: bool = False):
    ...
    extrap_r2 = [r['evaluation']['r2'] for r in all_results if r.get('is_extrapolation_test')]
    ...
    print(f"\n📊 Performance:")
    print(f"   ⭐ Excellent (>0.99): {excellent:2d} ({100*excellent/len(r2_scores):5.1f}%)")
    print(f"   ✅ Very Good (>0.95): {good:2d} ({100*good/len(r2_scores):5.1f}%)")
    print(f"   🟡 Acceptable (>0.80): {acceptable:2d} ({100*acceptable/len(r2_scores):5.1f}%)")
    ...
    for decision in ['llm', 'nn', 'failed']:
        if decision in decisions:
            ...
            dec_r2 = [r['evaluation']['r2'] for r in all_results if r['decision'] == decision]
            mean_r2 = np.mean(dec_r2) if dec_r2 else 0
            print(f"   {decision.upper():8s}: {count:2d} ({pct:5.1f}%) - Mean R² = {mean_r2:.4f}")

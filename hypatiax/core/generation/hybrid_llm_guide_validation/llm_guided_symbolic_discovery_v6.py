...
✅ Configurable number of hypothesis iterations (--niterations)
...
Architecture:
    ...
    Phase 2: LLM Hypothesis Generation (5s)
        └─ Generate N candidate equations using domain knowledge
    ...
    def __init__(self, config: Optional[LLMConfig] = None, api_key: Optional[str] = None):
        ...
        if self.config.provider == "anthropic":
            try:
                ...
                print(f"   ✓ Generating {self.config.n_iterations} hypothesis candidates per test")
            except ImportError:
...
class LLMGuidedDiscovery:
    ...
    def discover(self, X: np.ndarray, y: np.ndarray, variable_names: List[str],
                ...
                n_hypotheses: Optional[int] = None, success_threshold: float = 0.95,
        ...
        hypotheses = self.hypothesis_generator.generate_hypotheses(
            domain, variable_names, variable_descriptions, description, patterns, n_hypotheses
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='LLM-Guided Symbolic Discovery v5.0',

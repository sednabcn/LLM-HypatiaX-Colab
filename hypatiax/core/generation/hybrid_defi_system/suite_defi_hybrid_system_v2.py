...
class SessionManager:
    ...
    def load_all_results(self) -> Dict[str, Dict]:
        ...
        for test_file in self.session_dir.glob("*.json"):
            if test_file.name in ['checkpoint.json', 'summary.json', 'complete_results.json']:
                continue
...
def convert_defi_protocol_to_test_cases(protocol: DeFiExperimentProtocolExtended, 
                                        domains: Optional[List[str]] = None) -> Dict[str, Dict]:
...
def list_test_cases_by_domain(test_cases: Dict):
    ...
    for domain in sorted(domains):
        cases = {name: case for name, case in test_cases.items() if case["domain"] == domain}
...
def run_single_test(test_name: str, test_cases: Dict, n_samples: int = 1000,
    ...
    try:
        ...
        discovery_config = DiscoveryConfig(
            ...
            auto_config_correlation_threshold=SYMBOLIC_CONFIG['auto_config_correlation_threshold'],
        )
        ...
        passed = (
            (discovery_r2 > 0.99 and val_score > 30.0) or
            (discovery_r2 > 0.95 and val_score > 80.0) or
            val_passed
        )
...
def run_all_tests_with_resume(test_cases: Dict, n_samples: int = 1000,
                              seed: Optional[int] = None, verbose: bool = True,
                              resume: bool = False, skip_tests: List[str] = None,
...
def generate_summary(results: Dict[str, Dict], total_time: float, 
    ...
    summary = {
        ...
        'passed': 0,
        'failed': 0,
        'by_domain': defaultdict(lambda: {'passed': 0, 'failed': 0, 'extrapolation': 0}),
    ...
    for test_name, result in results.items():
        ...
        detailed_entry = {
            ...
            'validation_score': validation.get('total_score', validation.get('overall_score', 0.0)),
...
def print_summary(summary: Dict):
    ...
    if summary.get('by_domain'):
        ...
        for domain, stats in summary['by_domain'].items():
            ...
            extrap = f" ({stats['extrapolation']} extrap)" if stats['extrapolation'] > 0 else ""
            print(f"  {domain:20s}: {stats['passed']:2d}/{total_domain:2d} ({rate:5.1f}%){extrap}")
    ...
    if summary.get('detailed_results'):
        ...
        print(f"{'='*120}")
        print(f"{'Test Case':<40} | {'R²':>6} | {'Val':>5} | {'Time':>6} | {'Status':>6} | {'Observations':<40}")
        print(f"{'-'*120}")
        ...
        for result in summary['detailed_results']:
            ...
            print(f"{test_name:<40} | {r2:>6.4f} | {val_score:>5.1f} | {time_sec:>5.1f}s | {status:>6} | {obs_str:<40}")
        
        print(f"{'='*120}")
        
        # Summary statistics
        r2_values = [r['r2'] for r in summary['detailed_results'] if 'r2' in r and not r.get('error')]
        val_values = [r['validation_score'] for r in summary['detailed_results'] if 'validation_score' in r and not r.get('error')]
...
Examples:
  ...
  python suite_defi_hybrid_system.py --list
    ...
    parser.add_argument('--domain', type=str, 
                       choices=['amm', 'risk_var', 'liquidity', 'expected_shortfall', 
                               'liquidation', 'staking'],
    ...
    parser.add_argument('--iterations', type=int,
                       help='Custom number of iterations (overrides mode)')
    parser.add_argument('--populations', type=int,
                       help='Custom population size')
    ...
    parser.add_argument('--force', action='store_true',
                       help='Force rerun even if test completed')
    
    # Information
    parser.add_argument('--list', action='store_true',
                       help='List all available tests')
    parser.add_argument('--list-sessions', action='store_true',
                       help='List previous test sessions')

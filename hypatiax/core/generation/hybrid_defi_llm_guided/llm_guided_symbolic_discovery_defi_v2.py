...
def print_results_table(results: Dict[str, Dict], test_cases: Dict):
    ...
    print(f"{'='*120}")
    print(
        f"{'Test Name':<40} | {'R²':>6} | {'Val':>5} | {'Time':>6} | {'Status':>6} | {'Notes':<30}"
    )
    print(f"{'-'*120}")
    ...
    for test_name, result in sorted_tests:
        ...
        print(
            f"{test_name_short:<40} | {r2:>6.4f} | {val:>5.1f} | {time_taken:>5.1f}s | {status:>6} | {notes_str:<30}"
        )
...
def main():
    ...
    if args.batch:
        ...
        result = run_single_test(
            args.test, test_cases, args.mode, api_key, args.niterations
        )

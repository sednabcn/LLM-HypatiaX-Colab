        ...
        for test_name, result in all_results.items():
            test_export = {
                'metadata': {
                    ...
                    'execution_time_seconds': result.get('timing', {}).get('total', 0.0)
                },
                ...
                'variables': {
                    ...
                    'descriptions': result.get('test_config', {}).get('variable_descriptions', {})
                },
...
def print_results_table(results: Dict[str, Dict], test_cases: Dict):
    ...
    print(f"{'='*120}")
    print(f"{'Test Name':<40} | {'R²':>6} | {'Val':>5} | {'Time':>6} | {'Status':>6} | {'Observations':<40}")
    print(f"{'-'*120}")
    ...
    for test_name, result in sorted_tests:
        ...
        print(f"{test_name_short:<40} | {r2:>6.4f} | {val:>5.1f} | {time_taken:>5.1f}s | {status:>6} | {obs_str:<40}")
    ...
    if total > 0:
        r2_values = [r.get("r2_score", 0) for r in results.values() if not r.get('error')]
        val_values = [r.get("validation_score", 0) for r in results.values() if not r.get('error')]
...
Examples:
  ...
  python llm_guided_symbolic_discovery_defi.py --domain amm --batch
    ...
    print(f"Difficulty: Easy: {stats['difficulty']['easy']} | "
    ...
    if args.list:
        ...
        for domain in protocol.get_all_domains():
            domain_tests = [name for name, tc in test_cases.items() if tc['domain'] == domain]
            if domain_tests:

...
class FormulaRegistry:
    ...
    def search(self, query: str) -> List[FormulaMetadata]:
        ...
        for formula in self.formulas.values():
            if (query_lower in formula.description.lower() or
                query_lower in formula.name.lower() or
                query_lower in formula.category.lower()):
    ...
    def _register_defi_formulas(self):
        ...
        self.register(FormulaMetadata(
            ...
            inputs=[
                FormulaInput(name="reserve_x", description="Token X reserve", unit="tokens", type="float", min_value=0),
                FormulaInput(name="reserve_y", description="Token Y reserve", unit="tokens", type="float", min_value=0)
            ],
            ...
            examples=[
                {"inputs": {"reserve_x": 1000, "reserve_y": 2000}, "output": 2000000}
            ]
        ))
    ...
    def _register_risk_formulas(self):
        ...
        self.register(FormulaMetadata(
            ...
            inputs=[
                ...
                FormulaInput(name="t", description="Time horizon (days)", unit="days", type="float", min_value=0, default=1)
            ],
            output_unit="percentage",
        ...
        self.register(FormulaMetadata(
            ...
            inputs=[
                FormulaInput(name="return_portfolio", description="Portfolio return", unit="percentage", type="float"),
                FormulaInput(name="risk_free_rate", description="Risk-free rate", unit="percentage", type="float", default=0.02),
                FormulaInput(name="volatility", description="Portfolio volatility", unit="percentage", type="float", min_value=0)
            ],
            output_unit="ratio",

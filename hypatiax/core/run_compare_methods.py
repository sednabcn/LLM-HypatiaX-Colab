...
def compare_methods():
    ...
    hybrid_r2_scores = [r.get('discovery', {}).get('r2_score', 0) for r in hybrid 
    ...
    comparison = {
        ...
        'Formulas Generated': [
            ...
            llm_formulas,
            len(nn),
            5  # Typical expert-derived formulas in DeFi
        ],
        'Validation Rate': [
    ...
    print("\nKEY FINDINGS:")
    print("-" * 80)
    print(f"1. Hybrid method generated {hybrid_formulas} formulas with "
          f"{format_percentage(hybrid_validation_rate)} validation rate")
    print(f"2. Pure LLM approach shows high extrapolation error "
    ...
    detailed_metrics = {
        'hybrid': {
            ...
            'avg_extrapolation_error': float(hybrid_avg_extrap) if isinstance(hybrid_avg_extrap, (int, float)) else None
        },
        'llm': {

...
def main():
    ...
    for domain_type, filename in file_mapping["llm"].items():
        ...
        if filepath.exists():
            ...
            if isinstance(data, list):
                ...
                print(
                    f"    ✅ Added {len(data)} cases from {domain_type}: {sorted(domains)}"
                )
        else:
    ...
    print("\nRun comparison with:")
    print(
        f"  python comparison_analysis_improved.py {output_llm.name} {output_nn.name}"
    )
    print("\nOr from the results directory:")
    print(
        f"  cd results && python ../comparison_analysis_improved.py {output_llm.name} {output_nn.name}"
    )

...
def create_comparison_tables(llm_results: List, nn_results: List):
    ...
    for i, (llm_r, nn_r) in enumerate(zip(llm_results, nn_results)):
        ...
        if isinstance(llm_r, dict):
            ...
            domain = llm_r.get(
                "domain", llm_r.get("metadata", {}).get("domain", "unknown")
            )
...
def plot_overall_comparison(df: pd.DataFrame, output_dir: Path):
    ...
    fig.suptitle(
        "Pure LLM vs Neural Network: Overall Comparison", fontsize=16, fontweight="bold"
    )
    ...
    ax2 = axes[0, 1]
    ax2.scatter(
        df["LLM R²"], df["NN R²"], alpha=0.6, s=100, edgecolors="black", linewidth=1
    )
...
def plot_domain_comparison(df: pd.DataFrame, output_dir: Path):
    ...
    domain_stats = (
        df.groupby("Domain")
        .agg({"LLM R²": ["mean", "std", "count"], "NN R²": ["mean", "std"]})
        .round(4)
    )
    ...
    ax1.set_title(
        "Mean R² by Domain (with standard deviation)", fontsize=14, fontweight="bold"
    )
    ax1.set_xticks(x)
...
def plot_formula_type_comparison(df: pd.DataFrame, output_dir: Path):
    ...
    formula_stats = (
        df.groupby("Formula Type")
        .agg({"LLM R²": ["mean", "count"], "NN R²": "mean", "R² Difference": "mean"})
        .round(4)
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "formula_type_comparison.png", dpi=300, bbox_inches="tight"
    )
...
def create_summary_tables(
    ...
    with open(output_dir / "summary_tables.txt", "w") as f:
        ...
        f.write(
            f"{'Moderate (0.80 < R² ≤ 0.95)':<40} {((llm_r2 > 0.80) & (llm_r2 <= 0.95)).sum():>25} {((nn_r2 > 0.80) & (nn_r2 <= 0.95)).sum():>25}\n"
        )
        f.write(
            f"{'Poor (R² ≤ 0.80)':<40} {(llm_r2 <= 0.80).sum():>25} {(nn_r2 <= 0.80).sum():>25}\n"
        )
        ...
        f.write("-" * 100 + "\n")
        f.write(
            f"{'Domain':<20} {'Count':>10} {'LLM Mean R²':>15} {'NN Mean R²':>15} {'LLM Advantage':>15}\n"
        )
        f.write("-" * 100 + "\n")
        ...
        for domain in domain_stats.index:
            ...
            f.write(
                f"{domain:<20} {count:>10} {llm_mean:>15.4f} {nn_mean:>15.4f} {advantage:>15.4f}\n"
            )
        ...
        f.write("-" * 100 + "\n")
        f.write(
            f"{'Formula Type':<25} {'Count':>10} {'LLM Mean R²':>15} {'NN Mean R²':>15} {'LLM Advantage':>15}\n"
        )
        f.write("-" * 100 + "\n")
        ...
        for ftype in formula_stats.index:
            ...
            f.write(
                f"{ftype:<25} {count:>10} {llm_mean:>15.4f} {nn_mean:>15.4f} {advantage:>15.4f}\n"
            )
        ...
        f.write("-" * 100 + "\n")
        f.write(
            f"{'Description':<50} {'LLM R²':>12} {'NN R²':>12} {'Difference':>12} {'Winner':>10}\n"
        )
        f.write("-" * 100 + "\n")
        ...
        for idx, row in df.iterrows():
            ...
            llm_r2_str = (
                f"{llm_r2:.4f}" if llm_r2 is not None and not pd.isna(llm_r2) else "N/A"
            )
            nn_r2_str = (
                f"{nn_r2:.4f}" if nn_r2 is not None and not pd.isna(nn_r2) else "N/A"
            )
            diff_str = (
                f"{diff:.4f}" if diff is not None and not pd.isna(diff) else "N/A"
            )
            f.write(
                f"{desc:<50} {llm_r2_str:>12} {nn_r2_str:>12} {diff_str:>12} {winner:>10}\n"
            )
...
def generate_comparison_report(llm_file: str, nn_file: str):
    ...
    print(
        f"    LLM: {(llm_r2 > 0.99).sum()}/{len(llm_r2)} ({(llm_r2 > 0.99).sum()/len(llm_r2)*100:.1f}%)"
    )
    print(
        f"    NN:  {(nn_r2 > 0.99).sum()}/{len(nn_r2)} ({(nn_r2 > 0.99).sum()/len(nn_r2)*100:.1f}%)"
    )

    print(f"\n  Good (R² > 0.95):")
    print(
        f"    LLM: {(llm_r2 > 0.95).sum()}/{len(llm_r2)} ({(llm_r2 > 0.95).sum()/len(llm_r2)*100:.1f}%)"
    )
    print(
        f"    NN:  {(nn_r2 > 0.95).sum()}/{len(nn_r2)} ({(nn_r2 > 0.95).sum()/len(nn_r2)*100:.1f}%)"
    )
    ...
    for domain in domain_stats.index:
        ...
        print(
            f"  Advantage:   {advantage:+.4f} {'(LLM wins)' if advantage > 0 else '(NN wins)' if advantage < 0 else '(Tie)'}"
        )
    ...
    best_llm = df[df["R² Difference"] > 0.2].sort_values(
        "R² Difference", ascending=False
    )
    if len(best_llm) > 0:
        ...
        for idx, row in best_llm.iterrows():
            ...
            print(
                f"    LLM: {row['LLM R²']:.4f} vs NN: {row['NN R²']:.4f} (Δ = +{row['R² Difference']:.4f})"
            )
    ...
    if len(best_nn) > 0:
        ...
        for idx, row in best_nn.iterrows():
            print(f"  • {row['Description'][:60]}")
            print(
                f"    NN: {row['NN R²']:.4f} vs LLM: {row['LLM R²']:.4f} (Δ = {row['R² Difference']:.4f})"
            )
    ...
    if llm_r2.mean() > nn_r2.mean() + 0.1:
        ...
        print(
            f"  • {winner_counts.get('LLM', 0)/total*100:.1f}% win rate vs {winner_counts.get('NN', 0)/total*100:.1f}%"
        )
        ...
        print("  • Overall superior performance across domains")
        print(
            f"  • {winner_counts.get('NN', 0)/total*100:.1f}% win rate vs {winner_counts.get('LLM', 0)/total*100:.1f}%"
        )
    ...
    for domain in domain_stats.index:
        summary["by_domain"][domain] = {
            ...
            "advantage": float(
                domain_stats.loc[domain, ("LLM R²", "mean")]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python comparison_analysis_improved.py <llm_results.json> <nn_results.json>"
        )

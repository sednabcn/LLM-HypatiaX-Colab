...
def load_actual_data():
    ...
    tests_data = [
        ...
        # Chemistry
        ("Arrhenius", "chemistry", 1.0000, 0.9996, 0.9989, 4, 12),  # exp(-E/(RT)) vs complex polynomial
        ("Henderson-Hasselbalch", "chemistry", 1.0000, 0.9985, 0.9989, 3, 15),  # pKa + log vs complex
        ("Rate Law", "chemistry", 1.0000, 0.9994, 1.0000, 3, 4),  # k[A]²[B]
        
        # Biology  
        ("Allometric Scaling", "biology", 1.0000, 0.9999, 1.0000, 2, 18),  # aM^b vs complex
        ...
        ("Gravitational Force", "physics", 1.0000, 0.2448, -0.0257, 4, 1),  # Failed badly
    ...
    results = []
    for name, domain, r2_llm, r2_nn, r2_hybrid, gt_complexity, hybrid_complexity in tests_data:
        results.append({
            'name': name,
            'domain': domain,
            'r2_llm': r2_llm,
...
def create_main_scatter():
    ...
    for item in data:
        ...
        ax.scatter(item['complexity_ground_truth'], item['r2_llm'],
                  marker='o', s=200, c=color, alpha=0.9,
                  edgecolors='black', linewidth=2, zorder=4,
                  label=f"LLM - {item['domain']}" if item['name'] == 
                  [d for d in data if d['domain'] == item['domain']][0]['name'] else "")
    ...
    for idx, item in enumerate(data):
        ...
        ax.scatter(item['complexity_ground_truth'] - nn_x_offset, item['r2_nn'],
                  marker='s', s=200, c=color, alpha=0.9,
                  edgecolors='black', linewidth=2, zorder=4)
    ...
    if failed:
        ax.scatter([7], [0.02], marker='X', s=300, c='red', 
                  edgecolors='black', linewidth=2, zorder=5,
                  label='Hybrid Failed (2 tests)')
        ax.text(7.5, 0.02, '2 Failed\n(M-M, Log)', fontsize=9, 
               va='center', fontweight='bold', color='red')
    
    # Reference lines
    ax.axhline(y=0.99, color='green', linestyle='--', linewidth=2.5, alpha=0.7,
              label='Excellent (R² ≥ 0.99)', zorder=1)
    ax.axhline(y=0.95, color='orange', linestyle='--', linewidth=2.5, alpha=0.7,
    ...
    ax.text(1.5, 1.02, 'Simple', fontsize=10, ha='center', fontweight='bold', color='green')
    ax.text(4, 1.02, 'Medium', fontsize=10, ha='center', fontweight='bold', color='orange')
    ax.text(12, 1.02, 'Complex', fontsize=10, ha='center', fontweight='bold', color='red')
    
    # Labels and styling
    ax.set_xlabel('Formula Complexity (operator count in ground truth equation)', 
    ...
    method_handles = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', 
                  markersize=14, label='Pure LLM (at ground truth)', 
                  markeredgecolor='black', markeredgewidth=2),
        plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='gray',
                  markersize=14, label='Neural Network (slightly left)', 
                  markeredgecolor='black', markeredgewidth=2),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='gray',
                  markersize=14, label='Hybrid v40 (actual complexity)', 
                  markeredgecolor='black', markeredgewidth=2),
    ...
    ax.legend(handles=domain_handles, loc='upper right',
             title='Domain', framealpha=0.95, fontsize=11, title_fontsize=12)
    ...
    ax.text(0.02, 0.58, textstr, transform=ax.transAxes, fontsize=10,
...
def print_summary_statistics():
    ...
    for item in data:
        ...
        print(f"{item['name']:<25} {item['domain']:<12} {gt:<4} {str(hybrid):<7} "
    ...
    successful_hybrid = [d for d in data if d['complexity_hybrid'] is not None 
                        and d['r2_hybrid'] is not None and d['r2_hybrid'] > 0.95]
    
    if successful_hybrid:
        inflation = [d['complexity_hybrid'] - d['complexity_ground_truth'] 

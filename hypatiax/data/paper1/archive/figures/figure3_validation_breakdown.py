...
def create_validation_breakdown():
    ...
    bars = ax.barh(layers, weights, color=colors, alpha=0.8, edgecolor='black', linewidth=1.2)
    ...
    bars1 = ax.bar(x - width/2, pure_llm_pass, width, 
                   label='Pure LLM', color='#E63946', alpha=0.8, 
                   edgecolor='black', linewidth=1.2)
    bars2 = ax.bar(x + width/2, hybrid_pass, width,
                   label='Hybrid v40', color='#06A77D', alpha=0.8,
                   edgecolor='black', linewidth=1.2)
    ...
    layer_names_short = ['Dimensional\nAnalysis', 'Domain\nConstraints', 
                         'Numerical\nStability', 'Extrapolation']
    y_positions = [8.5, 6.5, 4.5, 2.5]
    box_height = 1.2
    box_width = 8
    
    for i, (name, y_pos, color) in enumerate(zip(layer_names_short, y_positions, colors)):
        # Draw box
        rect = Rectangle((1, y_pos - box_height/2), box_width, box_height,
                         facecolor=color, edgecolor='black', linewidth=2, alpha=0.7)
        ax.add_patch(rect)
        
        # Add text
        ax.text(5, y_pos, name, ha='center', va='center', 
               fontsize=10, fontweight='bold', color='white')
        
        # Add pass rate
        pass_rate = hybrid_pass[i]
        ax.text(9.5, y_pos, f'{pass_rate*100:.0f}%', ha='left', va='center',
               fontsize=9, fontweight='bold', color=colors[i])
        
        # Draw arrow to next layer (except for last)
        if i < len(layer_names_short) - 1:
            ax.arrow(5, y_pos - box_height/2 - 0.1, 0, -0.7,
                    head_width=0.4, head_length=0.2, fc='black', ec='black', linewidth=1.5)
    
    # Add "Expression" input at top
    ax.text(5, 9.8, 'Discovered Expression', ha='center', va='center',
           fontsize=11, fontweight='bold', 
           bbox=dict(boxstyle='round,pad=0.5', facecolor='lightblue', edgecolor='black', linewidth=2))
    ax.arrow(5, 9.5, 0, -0.5, head_width=0.4, head_length=0.2, 
            fc='black', ec='black', linewidth=1.5)
    
    # Add "Validated Result" at bottom
    ax.text(5, 1.5, 'Validated Result', ha='center', va='center',
           fontsize=11, fontweight='bold',
...
def create_validation_breakdown_alternative():
    ...
    p1 = ax.bar(x, pass_pct, width, label='Pass', color='#06A77D', 
                edgecolor='black', linewidth=1.2)
    p2 = ax.bar(x, fail_pct, width, bottom=pass_pct, label='Fail', 
                color='#E63946', edgecolor='black', linewidth=1.2)
    p3 = ax.bar(x, na_pct, width, bottom=np.array(pass_pct)+np.array(fail_pct),
                label='N/A', color='#CCCCCC', edgecolor='black', linewidth=1.2)

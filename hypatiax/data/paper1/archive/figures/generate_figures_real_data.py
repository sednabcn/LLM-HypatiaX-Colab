...
def figure1_arrhenius_real(data):
    ...
    ax.axvspan(
        746, 1119, alpha=0.15, color=COLORS["vermillion"], label="2× Extrap (746-1119K)"
    )
    ...
    ax.plot(
        ...
        linewidth=2,
        label="Neural Network (R²=0.9997 train)",
        zorder=2,
    )
    ax.plot(
        T_all,
    ...
    ax.annotate(
        ...
        fontsize=11,
        bbox=dict(
            boxstyle="round", facecolor=COLORS["blue"], alpha=0.3, edgecolor="black"
        ),
    )
    ...
    ax.set_title(
        "Extrapolation Failure: Arrhenius Equation", fontsize=16, weight="bold", pad=20
    )

    plt.tight_layout()
    plt.savefig(
        "figures/figure1_arrhenius_extrapolation.pdf", dpi=300, bbox_inches="tight"
    )
    plt.savefig(
        "figures/figure1_arrhenius_extrapolation.png", dpi=300, bbox_inches="tight"
    )
...
def figure5_method_comparison_real(data):
    ...
    time = [
        ...
        np.mean(
            [t for t in core_results["hybrid_v40"]["time"] if t < 200]
        ),  # ~45s (exclude gravity outlier)
    ]
    ...
    r2 = [
        ...
        np.mean(
            [r for r in core_results["hybrid_v40"]["r2"] if r > 0]
        ),  # 0.9996 (exclude failures)
    ]
    ...
    extrap = [
        (
            np.mean(extrap_data["neural_net"]["errors"])
            if extrap_data["neural_net"]["errors"]
            else 3348
        ),
        float("inf"),  # Pure LLM can't extrapolate formulas
        (
            np.mean(extrap_data["hybrid_v40"]["errors"])
            if extrap_data["hybrid_v40"]["errors"]
            else 0.1
        ),
    ]
    ...
    scatter1 = ax1.scatter(
        time, r2, c=colors, s=400, alpha=0.8, edgecolors="black", linewidth=2, zorder=3
    )
    ...
    for i, m in enumerate(methods):
        ...
        ax2.annotate(
            label,
            (time[i], extrap_plot[i]),
            xytext=xytext,
            textcoords="offset points",
            fontsize=10,
            weight="bold",
            bbox=dict(
                boxstyle="round", facecolor=facecolor, alpha=0.9, edgecolor="black"
            ),
        )
    ...
    bars = ax3.bar(
        methods, interpretable, color=colors, alpha=0.8, edgecolor="black", linewidth=2
    )
    ...
    plt.suptitle(
        "Method Comparison: Real Experimental Results",
        fontsize=16,
        weight="bold",
        y=1.02,
    )

...
class ExtrapolationErrorFixer:
    ...
    WRONG_PATTERN = re.compile(
        r'(\s*extrap_error(?:_\w+)?\s*=\s*\([^)]+\)\s*\*\s*100)',
        re.MULTILINE
    )
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.fixes_applied = []
        self.backup_dir = self.project_root / 'backups' / f'extrap_fix_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    def apply_fix(self, file_path: Path, dry_run: bool = True):
        ...
        for match in reversed(matches):
            ...
            fixed_content = fixed_content[:match.start()] + new_text + fixed_content[match.end():]
    ...
    def generate_report(self):
        ...
        print("\n📝 What changed:")
        print("  BEFORE: extrap_error = (rmse_extrap / rmse_train) * 100  # 3348% = 33.48×")

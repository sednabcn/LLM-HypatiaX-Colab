...
class ExtrapolationErrorSearcher:
    ...
    PATTERNS = [
        ...
        (r'(\w+_?error\w*)\s*=\s*\(([^)]+)\)\s*\*\s*100', 
         'simple_multiply'),
        
        # Pattern 2: Within if/ternary
        (r'(\w+_?error\w*)\s*=\s*\(([^)]+)\)\s*\*\s*100\s+if\s+', 
         'ternary_multiply'),
        
        # Pattern 3: Direct calculation in context
        (r'\(\s*(\w+)\s*/\s*(\w+)\s*\)\s*\*\s*100', 
         'inline_multiply'),
        
        # Pattern 4: With variable capture
        (r'=\s*\(\s*rmse_\w+\s*/\s*rmse_\w+\s*\*\s*100\s*\)',
         'rmse_ratio_multiply'),
    ]
    
    # Keywords that indicate this is extrapolation-related
    EXTRAP_KEYWORDS = [
        'extrap', 'extrapolation', 'rmse_train', 'rmse_test',
        'train_error', 'test_error', 'degradation'
    ]
    ...
    def _search_file(self, file_path: Path) -> List[Dict]:
        ...
        for pattern, pattern_type in self.PATTERNS:
            for match in re.finditer(pattern, content, re.MULTILINE):
                ...
                is_extrap = any(kw in context.lower() 
                               for kw in self.EXTRAP_KEYWORDS)
    ...
    def print_findings(self):
        ...
        for i, finding in enumerate(self.findings, 1):
            ...
            for line in finding['context'].split('\n'):
                marker = '➡️ ' if line.strip() == finding['line_content'].strip() else '   '
                print(f"   {marker}{line}")
            print()
    ...
    def _fix_file(self, file_path: Path, findings: List[Dict], 
                  dry_run: bool) -> bool:
        ...
        findings_sorted = sorted(findings, key=lambda x: x['line_num'], 
                               reverse=True)
        ...
        while backup_file.exists():
            backup_file = self.backup_dir / f"{file_path.stem}_{counter}{file_path.suffix}"
            counter += 1
...
Examples:
  ...
  python search_and_fix_all_extrap_errors.py --fix --project /path/to/project
    ...
    parser.add_argument(
        '--fix',
        action='store_true',
        help='Fix all issues found (creates backups)'
    )

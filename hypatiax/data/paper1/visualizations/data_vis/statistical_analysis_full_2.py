...
def main():
    ...
    if extrap_data_plot:
        ...
        bp1 = ax1.boxplot(extrap_data_plot, positions=positions, widths=0.5,
                          patch_artist=True, tick_labels=extrap_labels)
    ...
    if interp_data_plot:
        ...
        bp2 = ax2.boxplot(interp_data_plot, positions=positions2, widths=0.5,
                          patch_artist=True, tick_labels=interp_labels)
    ...
    if use_extrap or use_r2:
        ...
        if use_extrap:
            ...
            title = 'Benchmark Comparison: Extrapolation Performance in Medium Regime (2×)'
            format_str = '.1f'
            unit = '%'
        ...
        # Create violin plots
        parts = ax.violinplot([hybrid_data, nn_data], 
                              positions=[1, 2],
                              showmeans=True, 
                              showmedians=True,
                              widths=0.7)
        ...
        # Overlay box plots
        bp = ax.boxplot([hybrid_data, nn_data],
                        positions=[1, 2],
                        widths=0.4,
                        patch_artist=True,
                        showfliers=True)
        ...
        y_pos_hybrid = max(hybrid_data) * 1.05 if use_extrap else min(hybrid_data) - 0.02
        y_pos_nn = max(nn_data) * 1.05 if use_extrap else min(nn_data) - 0.02
        va = 'bottom' if use_extrap else 'top'
        
        ax.text(1, y_pos_hybrid, 
        ...
        try:
            ...
            sig_text = "Hybrid v40 > Neural Network" if use_r2 else "Hybrid v40 < Neural Network"
            ax.text(0.5, 0.95, 
        ...
        plt.savefig(pdf_file2, format='pdf', bbox_inches='tight', 
                    dpi=300, metadata={'Creator': 'Matplotlib', 'Title': 'Benchmark Comparison'})
        ...
        print("⚠️  Insufficient data for benchmark comparison (need Hybrid v40 and Neural Network data)")
    ...
    if (output_dir / "figure_benchmark_comparison.pdf").exists():
        pdf2_size = (output_dir / "figure_benchmark_comparison.pdf").stat().st_size / 1024
        png2_size = (output_dir / "figure_benchmark_comparison.png").stat().st_size / 1024

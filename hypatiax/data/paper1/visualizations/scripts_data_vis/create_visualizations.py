...
def extract_plot_data(data):
    ...
    for name, values in systems.items():
        print(f"  • {name:20s}: {len(values):2d} points (median: {np.median(values):.1f}%)")
...
def create_boxplot(systems, output_dir):
    ...
    ax.set_title('Extrapolation Performance Comparison',
    ...
    for pos, errors in zip(positions, data_to_plot):
        ...
        ax.text(pos, median * 1.15, f'{median:.1f}%',
               ha='center', va='bottom', fontsize=9, fontweight='bold')
        
        # N count below
        ax.text(pos, ax.get_ylim()[0] * 1.5, f'n={len(errors)}',
               ha='center', va='top', fontsize=8, style='italic')
    
    # Reference lines
    ax.axhline(y=10, color='green', linestyle='--', alpha=0.4, linewidth=1, label='10% (excellent)')
    ax.axhline(y=100, color='orange', linestyle='--', alpha=0.4, linewidth=1, label='100% (2× error)')
...
def create_barplot(systems, output_dir):
    ...
    ax.set_title('Median Performance with Interquartile Range',
                fontsize=14, fontweight='bold', pad=15)
    ax.set_yscale('log')
        ...
        for test in data.get('tests', []):
            if method_orig in test.get('results', {}):
                ...
                if 'extrapolation_errors' in result:
                    ...
                    if np.isfinite(r2) and np.isfinite(extrap) and -1 < r2 < 1.1 and extrap < 1e6:
                        r2_values.append(r2)
                        extrap_values.append(extrap)
        
        if len(r2_values) > 0:
            ax.scatter(r2_values, extrap_values, 
                      c=color, marker=marker, s=100, alpha=0.7,
                      label=f'{label} (n={len(r2_values)})', edgecolors='black')

...
class SystemDataExtractor:
    ...
    def process_all_files(self, pattern: str = "*test*.json") -> pd.DataFrame:
        ...
        print(f"   • Unidentified: {len(all_results) - system_2_count - system_3_count} tests")
    ...
    def create_extrapolation_comparison(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        summary = df_clean.groupby('system_type').agg({
    ...
    def generate_json_for_statistical_analysis(self, df: pd.DataFrame, 
                                               output_file: str = "systems_2_3_data.json"):
        """
        ...
        for test_name, group in df.groupby('test_name'):
            ...
            for _, row in group.iterrows():
                ...
                result = {
                    ...
                    "r2": float(row['r2_score']) if not pd.isna(row['r2_score']) else 0.0,
                    "rmse": np.inf,  # Not extracted, use infinity
                    "time": float(row['total_time']) if not pd.isna(row['total_time']) else 0.0,
                    "formula": str(row['expression']),
                    "error": None,
                    "metadata": {
                        "validation_score": float(row['validation_score']) if not pd.isna(row['validation_score']) else 0.0,
                ...
                if not pd.isna(row['extrap_near']):
                    result["extrapolation_errors"] = {
                        "near": float(row['extrap_near']) if row['extrap_near'] != np.inf else np.inf,
                        "medium": float(row['extrap_medium']) if row['extrap_medium'] != np.inf else np.inf,
                        "far": float(row['extrap_far']) if row['extrap_far'] != np.inf else np.inf
                    }

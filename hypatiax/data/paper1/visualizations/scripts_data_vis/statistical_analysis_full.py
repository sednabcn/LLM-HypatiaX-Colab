...
class UnifiedAnalyzer:
    ...
    def merge_all_data(self):
        ...
        print(f"✅ Loaded: {self.systems_23_file} ({systems_23_data['total_tests']} tests)")
        ...
        for test in interp_data['tests']:
            ...
            if test_name in test_map:
                for method in ["Pure LLM", "Neural Network", "Hybrid System v40"]:
                    if method in test['results'] and method in test_map[test_name]["results"]:
        ...
        if systems_2_path.exists():
            ...
            print(f"✅ Also loaded: {self.systems_2_file} ({systems_2_data.get('total_tests', 0)} tests)")
            ...
            for test in systems_data['tests']:
                ...
                for method_orig in test['results']:
                    ...
                    if method not in test_map[matched_test]["results"]:
                        test_map[matched_test]["results"][method] = test['results'][method_orig]
                    else:
                        ...
                        if 'extrapolation_errors' in new_result and 'extrapolation_errors' not in existing_result:
        ...
        for method, count in coverage.items():
            ...
            system_details[method] = [test['test_name'] for test in unified['tests'] 
                                     if method in test['results']]
        ...
        if coverage.get("System 2 Symbolic", 0) > 0:
            ...
            print(f"     Check if systems_2_3_data.json has 'System 2 Symbolic' results")
    ...
    def extract_data_for_analysis(self):
        ...
        systems = {
            "Pure_LLM": {"near_1.2x": [], "medium_2x": [], "far_5x": [], "r2_scores": []},
            "Neural_Network": {"near_1.2x": [], "medium_2x": [], "far_5x": [], "r2_scores": []},
            "Hybrid_v40": {"near_1.2x": [], "medium_2x": [], "far_5x": [], "r2_scores": []},
            "System_2_Symbolic": {"near_1.2x": [], "medium_2x": [], "far_5x": [], "r2_scores": []},
            "System_3_LLM_Fallback": {"near_1.2x": [], "medium_2x": [], "far_5x": [], "r2_scores": []}
        }
        ...
        for method_key in systems.keys():
            ...
            print(f"  {method_key:25s}: {n_r2:2d} R² scores, {n_extrap:2d} extrap tests")
    ...
    def run_statistical_tests(self):
        ...
        systems_with_extrap = []
        for system in ["Hybrid_v40", "Pure_LLM", "System_3_LLM_Fallback", 
        ...
        if "Hybrid_v40" in systems_with_extrap and "Neural_Network" in systems_with_extrap:
            comparisons = [
            ...
            if "System_3_LLM_Fallback" in systems_with_extrap:
                comparisons.append(("Hybrid_v40", "System_3_LLM_Fallback", "Hybrid v40 vs System 3"))
            if "System_2_Symbolic" in systems_with_extrap:
                comparisons.append(("Hybrid_v40", "System_2_Symbolic", "Hybrid v40 vs System 2"))
    ...
    def _save_basic_stats(self):
        ...
        for system in ["Hybrid_v40", "Pure_LLM", "System_3_LLM_Fallback", 
            ...
            desc_stats.append({
                ...
                'Extrap_Mean': np.mean(medium_data) if len(medium_data) > 0 else np.nan,
                'Extrap_Std': np.std(medium_data) if len(medium_data) > 0 else np.nan
            })
    ...
    def generate_visualizations(self):
        ...
        # Reference lines
        ax.axhline(y=10, color='green', linestyle='--', alpha=0.5, label='10% (excellent)')
        ax.axhline(y=100, color='orange', linestyle='--', alpha=0.5, label='100% (2× training error)')
        
        # Add mean values
        for i, (pos, errors) in enumerate(zip(positions, data_to_plot)):
            mean_val = np.mean(errors)
            ax.text(pos, mean_val * 1.5, f'{mean_val:.1f}%', 
        ...
        plt.savefig(png_file, format='png', bbox_inches='tight', 
                    dpi=300, metadata={'Software': 'Matplotlib'})
        plt.close()
...
"""
        ...
        for key, name, desc in systems:
            ...
            if len(medium) > 0:
                ...
                # Bold best result
                all_means = [np.mean(self.results[s[0]]["medium_2x"]) 
                            for s in systems if len(self.results[s[0]]["medium_2x"]) > 0]

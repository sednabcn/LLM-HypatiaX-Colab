...
def regenerate_figure_5systems():
    ...
    positions = [1, 2]
    bp1 = ax1.boxplot(extrap_data, positions=positions, widths=0.5,
                      patch_artist=True, labels=systems_extrap)
    bp1['boxes'][0].set_facecolor('blue')
    bp1['boxes'][1].set_facecolor('red')
    
    ax1.set_ylabel('Extrapolation Error (%)', fontsize=11)
    ax1.set_title('Extrapolation Performance (2× training range)', fontsize=12, fontweight='bold')
    ax1.set_yscale('log')
    ...
    positions2 = [1, 2, 3]
    bp2 = ax2.boxplot(interp_data, positions=positions2, widths=0.5,
                      patch_artist=True, labels=systems_interp)

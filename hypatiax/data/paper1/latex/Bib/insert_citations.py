...
class Citation:
    ...
    def __str__(self):
        return (f"Line {self.line_number}: \\{self.citation_style}{{{self.key}}} "
...
class BibEntry:
    ...
    def from_bibtex(cls, key: str, fields: Dict[str, str]):
        ...
        # Remove stopwords
        stopwords = {'from', 'with', 'that', 'this', 'were', 'have', 'been', 
                    'their', 'which', 'about', 'other', 'into', 'through'}
...
class CitationMatcher:
    ...
    STRONG_KEYWORDS = {
        'neural networks', 'deep learning', 'machine learning', 'symbolic regression',
        'extrapolation', 'generalization', 'physics-informed', 'language models',
        'llm', 'gpt', 'transformer', 'reasoning', 'discovery', 'scientific',
        'dimensional analysis', 'pareto', 'genetic programming', 'evolutionary',
        'interpretability', 'explainability', 'uncertainty', 'calibration'
    }
    ...
    def find_citations(self, latex_content: str) -> List[Citation]:
        ...
        for line_num, line in enumerate(lines, start=1):
            ...
            if any(cmd in line for cmd in ['\\begin{', '\\end{', '\\caption', '\\label']):
                continue
            ...
            for keyword in self.STRONG_KEYWORDS:
                if keyword in line_lower:
                    ...
                    relevant_keys = self._find_relevant_citations(keyword, existing_citations)
    ...
    def _determine_citation_style(self, line: str) -> str:
        ...
        if re.search(r'\b(showed|demonstrated|found|proposed|developed|presented)\b', line, re.I):
            return 'citet'
        
        # Use \citep{} for general statements
        return 'citep'
    
    def _calculate_confidence(self, line: str, keyword: str, citation_key: str) -> float:
...
def main():
    ...
    parser.add_argument('--latex', default='paper.tex',
                       help='LaTeX source file')
    parser.add_argument('--bib', default='new_entries.bib',
                       help='Bibliography with new entries')
    parser.add_argument('--output', default='paper_cited.tex',
                       help='Output LaTeX file with citations')
    parser.add_argument('--diff', default='citation_diff.txt',
                       help='Diff file showing changes')

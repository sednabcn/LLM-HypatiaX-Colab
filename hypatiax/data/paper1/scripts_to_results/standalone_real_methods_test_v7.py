...
# Import protocols
from extrapolation_test_protocol import ExtrapolationTestProtocol, REGIMES
...
class GroundTruthRegistry:
    ...
    @staticmethod
    def get_function(equation_name: str) -> Tuple[Callable, Dict[str, Tuple[float, float]]]:
        ...
        # DEFI
            ...
            return port_var, {"var1": (5000, 50000), "var2": (3000, 30000), "rho": (-0.5, 0.9)}
...
class TestResult:
    ...
    def to_dict(self) -> Dict:
        result_dict = {
            ...
            "extrapolation_errors": {k: float(v) for k, v in self.extrapolation_errors.items()},
...
class PureLLMMethodWrapper:
    ...
    def run(self, test_name, description, domain, X, y, var_names, metadata, verbose=False):
        if not self.baseline:
            return TestResult(
                method=self.name, test_name=test_name, domain=domain,
                success=False, r2=0.0, rmse=float('inf'), time=0.0,
        ...
        try:
            ...
            if "error" in result:
                return TestResult(
                    method=self.name, test_name=test_name, domain=domain,
                    success=False, r2=0.0, rmse=float('inf'),
                    time=time.time() - start, error=result["error"]
                )
            ...
            return TestResult(
                method=self.name, test_name=test_name, domain=domain,
                success=False, r2=0.0, rmse=float('inf'),
                time=time.time() - start, error=str(e)[:100]
            )
    ...
    def _predict(self, X, result, metadata):
        ...
        try:
            ...
            if len(predictions) != len(X):
                raise ValueError(f"Prediction length mismatch: {len(predictions)} != {len(X)}")
...
class NeuralNetworkMethodWrapper:
    ...
    def __init__(self):
        ...
        try:
            from hypatiax.core.training.baseline_neural_network import train_and_evaluate
    ...
    def _predict(self, X, result, metadata):
        ...
        try:
            ...
            # Validate output
            if len(y_pred) != len(X):
                raise ValueError(f"Prediction length mismatch: {len(y_pred)} != {len(X)}")
...
class HybridSystemV40Wrapper:
    ...
    def run(self, test_name, description, domain, X, y, var_names, metadata, verbose=False):
        ...
        try:
            ...
            test_result._prediction_cache['variable_names'] = var_names  # ORIGINAL names
    ...
    def _predict(self, X, result, metadata):
        ...
        try:
            ...
            if not var_names or len(var_names) != X.shape[1]:
                ...
                var_names = sorted(set(v for v in potential_vars 
                                      if v not in ['exp', 'log', 'sqrt', 'sin', 'cos', 'tan', 
                                                   'log10', 'abs', 'max', 'min']))[:X.shape[1]]
            
            if len(var_names) != X.shape[1]:
                raise ValueError(f"Variable count mismatch: {len(var_names)} names for {X.shape[1]} columns")
            ...
            func = sp.lambdify(list(symbols_dict.values()), expr_parsed, 
                             modules=['numpy', 'math'])
...
class StandaloneTestRunner:
    ...
    def run_all_domains(self, protocol: ComparativeExperimentProtocol, num_samples: int = 200, verbose: bool = True):
        """Run tests across all domains"""
        
        all_domains = protocol.get_all_domains()
        total_tests = sum(len(protocol.load_test_data(d, num_samples=10)) for d in all_domains)
        ...
        for domain_idx, domain in enumerate(all_domains, 1):
            print(f"\n{'='*80}")
            print(f"DOMAIN {domain_idx}/{len(all_domains)}: {domain.upper()}".center(80))
            print(f"{'='*80}")
    ...
    def run_test_with_extrapolation(
        ...
        for method in self.methods:
            ...
            if verbose:
                if result.success:
                    extrap_str = ""
                    if self.enable_extrapolation and "medium" in result.extrapolation_errors:
        ...
        for regime_name, (X_extrap, y_extrap) in extrapolation_data.items():
            try:
                ...
                if verbose:
                    ...
                    print(f"      [DEBUG] Cache keys: {list(result._prediction_cache.keys())}")
                ...
                if np.any(np.isnan(y_pred)):
                    raise ValueError(f"Prediction contains {np.sum(np.isnan(y_pred))} NaN values")
                
                if np.any(np.isinf(y_pred)):
                    raise ValueError(f"Prediction contains {np.sum(np.isinf(y_pred))} Inf values")
    ...
    def _print_domain_summary(self, domain: str):
        ...
        for test in domain_results:
            for method_name, result in test["results"].items():
                ...
                if result["success"]:
                    ...
                    if self.enable_extrapolation and "medium" in result.get("extrapolation_errors", {}):
                        err = result["extrapolation_errors"]["medium"]
                        if err < 10000:
        ...
        for method, stats in method_stats.items():
            ...
            print(f"  {method:<30} Success: {stats['successes']}/{total_tests} ({success_rate:5.1f}%), "
    ...
    def print_summary(self):
        ...
        for test in self.results:
            best_r2 = max(
                r["r2"] for r in test["results"].values() if r["success"]
            ) if any(r["success"] for r in test["results"].values()) else -1
        ...
        for method in sorted(method_stats.keys(), 
                            key=lambda x: method_stats[x]["successes"], reverse=True):
            stats = method_stats[method]
            success_rate = (stats["successes"] / total_tests * 100) if total_tests > 0 else 0
            bar = "█" * int(success_rate / 5)
            print(f"   {method:<35} {stats['successes']:>3}/{total_tests}  ({success_rate:>5.1f}%) {bar}")
        
        print(f"\n📈 Average R² (successful tests):")
        for method in sorted(method_stats.keys(),
                            key=lambda x: np.mean(method_stats[x]["r2_scores"]) if method_stats[x]["r2_scores"] else -1,
                            reverse=True):
            stats = method_stats[method]
            if stats["r2_scores"]:
        ...
        print(f"\n⏱️  Average Time:")
        for method in sorted(method_stats.keys(),
                            key=lambda x: np.mean(method_stats[x]["times"]) if method_stats[x]["times"] else float('inf')):
            stats = method_stats[method]
            if stats["times"]:
        ...
        for method in sorted(method_stats.keys(),
                            key=lambda x: method_stats[x]["wins"], reverse=True):
            stats = method_stats[method]
            win_rate = (stats["wins"] / total_tests * 100) if total_tests > 0 else 0
            print(f"   {method:<35} {stats['wins']:>3}/{total_tests}  ({win_rate:>5.1f}%)")
        ...
        if self.enable_extrapolation:
            ...
            for regime in ["near", "medium", "far"]:
                ...
                for method in sorted(method_stats.keys(),
                                    key=lambda x: np.mean(method_stats[x][f"extrap_{regime}"]) 
                                    if method_stats[x][f"extrap_{regime}"] else float('inf')):
                    stats = method_stats[method]
                    ...
                    if errors:
                        ...
                        print(f"   {method:<35} {avg_err:6.1f}% ± {std_err:5.1f}%  {status}  ({n_valid}/{n_total})")
                    else:
        ...
        for domain, stats in self.domain_stats.items():
            ...
            for method in sorted(stats.keys()):
                ...
                success_rate = (method_data["successes"] / domain_tests * 100) if domain_tests > 0 else 0
                avg_r2 = np.mean(method_data["r2_scores"]) if method_data["r2_scores"] else 0
                
                print(f"   {method:<30} {method_data['successes']}/{domain_tests} ({success_rate:5.1f}%), "
                      f"R²: {avg_r2:.4f}")
    ...
    def _generate_table1_data(self, method_stats):
        ...
        print("\\toprule")
        print("\\textbf{Method} & \\textbf{Accuracy (R²)} & \\textbf{Extrap. Error} & \\textbf{Correct Form} & \\textbf{Time} \\\\")
        print("\\midrule")
        ...
        for method, stats in sorted(method_stats.items()):
            ...
            print(f"{method} & ${avg_r2:.2f} \\pm {std_r2:.2f}$ & {avg_extrap:.0f}\\% & "
...
WHAT'S NEW IN V4:
  ...
  ✅ Proper filtering of invalid extrapolation errors
    ...
    parser.add_argument('--extrapolation', action='store_true',
                       help='Enable extrapolation testing')
    ...
    if args.all:
        ...
        print("  python standalone_real_methods_test_v4.py --test arrhenius --extrapolation")
        return

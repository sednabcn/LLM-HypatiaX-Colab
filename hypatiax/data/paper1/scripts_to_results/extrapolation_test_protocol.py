...
class ExtrapolationTestProtocol:
    ...
    def test_method_extrapolation(
        ...
        for regime in REGIMES:
            ...
            if verbose:
                print(f"\n🔬 {regime.description}:")
                print(f"   Range: [{self.x_max * regime.multiplier:.2f}, "
    ...
    def plot_extrapolation_results(
        ...
        ax1.axvspan(self.x_min, self.x_max, alpha=0.2, color='green', 
                    label='Training Range')
        
        for regime in REGIMES:
            r_min = self.x_max * regime.multiplier
            r_max = self.x_max * regime.multiplier * 1.5
            ax1.axvspan(r_min, r_max, alpha=0.1, 
                       label=f'{regime.name.capitalize()} Extrap.')
        ...
        for i, results in enumerate(results_list):
            ...
            ax2.bar(x_pos + i*width, rmses, width, 
                   label=results["method"], alpha=0.8)
        ...
        ax3 = axes[1, 0]
        for i, results in enumerate(results_list):
            errors = [results["regimes"][r]["extrapolation_error_pct"] 
        ...
        for results in results_list:
            errors = [results["regimes"][r]["extrapolation_error_pct"] 
                     for r in regime_names]
            ax4.plot(multipliers, errors, marker='o', linewidth=2,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test', type=str, default='arrhenius',
    ...
    if args.test == 'arrhenius':
        ...
        print(f"Hybrid Medium Extrapolation Error: "

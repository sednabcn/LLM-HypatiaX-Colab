...
class MethodResult:
    ...
    def to_dict(self) -> Dict:
        return {
            "method": self.method, "success": self.success,
            "r2": float(self.r2), "rmse": float(self.rmse),
            "formula": self.formula, "error": self.error,
            "time": float(self.time), "metadata": self.metadata or {}
        }

# VARIABLE NAME SANITIZER (FIX FOR S, N CONFLICTS)
class VariableNameSanitizer:
    RESERVED_NAMES = {'S', 'N', 'C', 'D', 'E', 'I', 'O', 'exp', 'log', 'sin', 'cos', 'sqrt'}
...
class BaseMethod:
    ...
    def run(self, description: str, X: np.ndarray, y: np.ndarray, 
            var_names: List[str], metadata: Dict, verbose: bool = False) -> MethodResult:
...
class LLMGuidedPySRSimple(BaseMethod):
    ...
    def run(self, description: str, X: np.ndarray, y: np.ndarray,
        ...
        if not PYSR_AVAILABLE:
            return MethodResult(self.name, False, 0.0, float('inf'), "N/A", "PySR not available")
        
        try:
            sanitizer = VariableNameSanitizer()
            sanitized_vars, _ = sanitizer.sanitize(var_names)
            
            guidance = {"operators": ["+", "-", "*", "/"], "unary_operators": ["exp", "log"]}
            
            model = PySRRegressor(
                niterations=50, binary_operators=guidance["operators"],
                unary_operators=guidance["unary_operators"],
                populations=10, population_size=30, maxsize=15,
                timeout_in_seconds=120, parsimony=0.001,
                random_state=42, verbosity=0, procs=0, multithreading=False
            )
            
            model.fit(X, y, variable_names=sanitized_vars)
            y_pred = model.predict(X)
            
            return MethodResult(self.name, True, self._safe_r2(y, y_pred),
                              self._safe_rmse(y, y_pred), str(model.get_best()))
        except Exception as e:
            return MethodResult(self.name, False, 0.0, float('inf'), "N/A", str(e)[:150])
...
class PurePySR(BaseMethod):
    ...
    def run(self, description: str, X: np.ndarray, y: np.ndarray,
        ...
        try:
            ...
            model = PySRRegressor(
                niterations=50, binary_operators=["+", "-", "*", "/", "pow"],
...
class PureLLMBasic(BaseMethod):
    ...
    def run(self, description: str, X: np.ndarray, y: np.ndarray,
            var_names: List[str], metadata: Dict, verbose: bool = False) -> MethodResult:
        if not self.client:
            return MethodResult(self.name, False, 0.0, float('inf'), "N/A", "No API key")
...
Use numpy as np."""
        ...
        try:
            ...
            if not code:
                return MethodResult(self.name, False, 0.0, float('inf'), "N/A", "No code")
            
            y_pred = self._safe_execute(code, X, var_names)
            if y_pred is None:
                return MethodResult(self.name, False, 0.0, float('inf'), "N/A", "Exec failed")
            ...
            return MethodResult(self.name, True, self._safe_r2(y, y_pred),
                              self._safe_rmse(y, y_pred), code[:80])
        except Exception as e:
            return MethodResult(self.name, False, 0.0, float('inf'), "N/A", str(e)[:150])
    ...
    def _safe_execute(self, code: str, X: np.ndarray, var_names: List[str]) -> Optional[np.ndarray]:
        try:
            import ast
            ast.parse(code)
            local_vars = {}
            exec(code, {"np": np, "numpy": np, "__builtins__": {"abs": abs, "max": max, "min": min}}, local_vars)
...
class NeuralNetworkMethod(BaseMethod):
    ...
    def run(self, description: str, X: np.ndarray, y: np.ndarray,
        ...
        try:
            ...
            if len(X) < 10:
                return MethodResult(self.name, False, 0.0, float('inf'), "N/A", "Need 10+ samples")
            ...
            model = nn.Sequential(
                nn.Linear(X.shape[1], hidden), nn.ReLU(), nn.Dropout(0.2),
            ...
            return MethodResult(self.name, True, self._safe_r2(y_test, y_pred),
                              self._safe_rmse(y_test, y_pred), f"NN({X.shape[1]}→{hidden}→1)")
        except Exception as e:
            return MethodResult(self.name, False, 0.0, float('inf'), "N/A", str(e)[:150])
...
class UltimateComparativeSuite:
    def __init__(self, verbose: bool = False):
        self.verbose = verbose
        self.methods = [
            LLMGuidedPySRSimple(verbose), PurePySR(verbose),
            PureLLMBasic(verbose), PureLLMEnhanced(verbose),
            NeuralNetworkMethod(verbose), LLMNNEnsembleSimple(verbose),
            LLMNNEnsembleSmart(verbose), IntegratedLLMDiscovery(verbose),
            HybridSystemV40(verbose)
        ]
        self.results = []
        print(f"\n{'='*80}\nULTIMATE SUITE - COMPLETE FIXED\n{'='*80}")
        print(f"Methods: {len(self.methods)} | PySR: {'✅' if PYSR_AVAILABLE else '❌'}\n{'='*80}\n")
    
    def run_test(self, description: str, X: np.ndarray, y: np.ndarray,
                 var_names: List[str], metadata: Dict, domain: str, verbose: bool = True) -> Dict:
        if verbose:
            print(f"\n{'='*80}\nTest: {description}\nDomain: {domain}\n{'='*80}")
        ...
        for i, method in enumerate(self.methods, 1):
            if verbose:
                print(f"[{i}/{len(self.methods)}] {method.name}...", end=" ", flush=True)
            start = time.time()
        ...
        self.results.append({"description": description, "domain": domain,
    ...
    def print_summary(self):
        ...
        for method_name, details in method_details.items():
            ...
            print(f"   Success Rate: {details['successes']}/{total} ({success_rate:.1f}%)")
            ...
            if details['r2_scores']:
                ...
                print(f"   R² Score:     {avg_r2:.4f} ± {std_r2:.4f}  (min: {min_r2:.4f}, max: {max_r2:.4f})")
            else:
            ...
            if details['times']:
                ...
                print(f"   Avg Time:     {avg_time:.2f}s per test (total: {total_time:.1f}s)")
        ...
        for i, (method, avg_r2, count) in enumerate(sorted(r2_rankings, key=lambda x: x[1], reverse=True), 1):
            medal = {1: "🥇", 2: "🥈", 3: "🥉"}.get(i, "  ")
        ...
        for i, (method, rate, count) in enumerate(sorted(success_rankings, key=lambda x: x[1], reverse=True), 1):
        ...
        for i, (method, avg_time) in enumerate(sorted(speed_rankings, key=lambda x: x[1]), 1):
        ...
        for domain in sorted(domains):
            ...
            for method, count in sorted(domain_wins.items(), key=lambda x: x[1], reverse=True)[:3]:
...
def main():
    ...
    if args.test:
        ...
        for desc, X, y, vars, meta in protocol.load_test_data(args.domain, args.samples):

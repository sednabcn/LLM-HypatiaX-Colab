...
class LLMGuidedPySRSimple(BaseMethod):
    ...
    def _get_llm_guidance(self, description: str, var_names: List[str], 
                         metadata: Dict) -> Dict:
        ...
        if any(var in RESERVED_JULIA for var in var_names):
            ...
            if self.verbose:
                ...
                print(f"⚠️  Variable name conflict detected: {conflicting} - using safe operators")
            return {"operators": default_operators, "unary_operators": default_unary}
...
FUNCTIONS: [exp, log]"""
        ...
        try:
            ...
            if ops_match:
                parsed_ops = [op.strip().strip("'\"") for op in ops_match.group(1).split(",") if op.strip()]
            ...
            if funcs_match:
                parsed_funcs = [f.strip().strip("'\"") for f in funcs_match.group(1).split(",") if f.strip()]
            ...
            if self.verbose:
                print(f"  LLM suggested operators: {operators}, functions: {unary_operators}")
        ...
        for line in lines:
            if line.strip().startswith('def '):
                ...
                if line.strip():
                    ...
                    if current_indent <= base_indent and not line[base_indent:].startswith(' '):
                        break
                code_lines.append(line)
        
        return '\n'.join(code_lines) if code_lines else ""
    
    def _safe_execute_code(self, code: str, X: np.ndarray, var_names: List[str]) -> Optional[np.ndarray]:
    ...
    def run(self, description: str, X: np.ndarray, y: np.ndarray,
        ...
        if llm_result.r2 > nn_result.r2:
            return MethodResult(
                ...
                metadata={"choice": "LLM", "llm_r2": llm_result.r2, "nn_r2": nn_result.r2}
            )
        else:
            return MethodResult(
                method=self.name,
                ...
                metadata={"choice": "NN", "llm_r2": llm_result.r2, "nn_r2": nn_result.r2}
            )
...
class UltimateComparativeSuite:
    ...
    def __init__(self, verbose: bool = False):
        ...
        print(f"Advanced Methods: {'✅' if ADVANCED_METHODS_AVAILABLE else '⚠️  (fallbacks enabled)'}")
        print(f"API Key: {'✅' if os.getenv('ANTHROPIC_API_KEY') else '❌'}")
        print(f"{'='*80}\n")
    
    def run_test(self, description: str, X: np.ndarray, y: np.ndarray,
                 var_names: List[str], metadata: Dict, domain: str, 
        ...
        for i, method in enumerate(self.methods, 1):
            if verbose:
                print(f"\n[{i}/{len(self.methods)}] Running {method.name}...", end=" ", flush=True)
            ...
            if verbose:
                if result.success:
                    print(f"✓ R²: {result.r2:.4f}, RMSE: {result.rmse:.4f}, Time: {result.time:.1f}s")
    ...
    def _print_comparison(self, results: Dict[str, MethodResult], 
                          comparison: Dict):
        ...
        for method_name, res in results.items():
            ...
            if res.success:
                print(f"{method_name:<45} {res.r2:<10.4f} {res.rmse:<12.6f} {res.time:<8.1f}s {rank:<6} {status:<20}")
            else:
                error = res.error[:15] if res.error else "Failed"
                print(f"{method_name:<45} {'N/A':<10} {'N/A':<12} {res.time:<8.1f}s {'-':<6} {error:<20}")
        ...
        if comparison["winner"] != "None":
            ...
            if comparison["advantages"]:
                ...
                for adv in comparison["advantages"][:5]:
                    print(f"     • {adv['method']}: +{adv['diff']:.4f} ({adv['pct']:.1f}% better)")
    ...
    def print_summary(self):
        ...
        print(f"\n📈 Average R² by method:")
        for method, r2_list in sorted(total_r2.items(), key=lambda x: np.mean(x[1]) if x[1] else -1, reverse=True):
            avg_r2 = np.mean(r2_list)
            std_r2 = np.std(r2_list)
            success_rate = (success_count[method] / total_tests) * 100
            print(f"   {method:<45} {avg_r2:.4f} ± {std_r2:.4f}  (success: {success_rate:.0f}%)")
        
        print(f"\n📉 Average RMSE by method:")
        for method, rmse_list in sorted(total_rmse.items(), key=lambda x: np.mean(x[1]) if x[1] else float('inf')):

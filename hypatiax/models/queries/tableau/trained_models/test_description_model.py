...
from hypatiax.custom_ner.queries.tableau import custom_tableau_desc_components
...
model_path = resources.files("hypatiax.models.queries.tableau.trained_models").joinpath(
    f"Description_sm_tableau_{niter}_{drop}_{batchsize}_data"
)
...
Test_desc_data = F.load("Test_tableau_desc_sm_data.json", "entity")
for item in Test_desc_data:
    print(item)

# Test
M = FilesManager("models", "queries", "tableau", "trained_models")
nlp = M.load(f"Description_sm_tableau_{niter}_{drop}_{batchsize}_data", "models")

    ...
    if 'defi' in df['domain'].values and 'risk' in df['domain'].values:
        ...
        if len(defi_scores) > 0 and len(risk_scores) > 0:
            ...
            tests['domain_comparison'] = {
                ...
                'effect_size_cohen_d': float((defi_scores.mean() - risk_scores.mean()) / 
...
def create_visualizations(df):
    ...
    axes[0, 0].axvline(df['total_score'].mean(), color='red', 
                       linestyle='--', label=f'Mean: {df["total_score"].mean():.1f}')
    ...
    axes[0, 1].axvline(df['r2_score'].mean(), color='red', 
                       linestyle='--', label=f'Mean: {df["r2_score"].mean():.3f}')
    ...
    bars = ax.bar(layers, layer_means, color=['skyblue', 'lightcoral', 'lightgreen'], 
...
def analyze_dataset():
    ...
    print(f"Valid formulas: {stats_dict['overall']['valid_formulas']} "

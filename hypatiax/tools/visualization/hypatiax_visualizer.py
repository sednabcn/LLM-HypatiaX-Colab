...
class HypatiaXVisualizer:
    ...
    def plot_il_over_time(self, historical_data, save_path='il_over_time.png'):
        ...
        ax1.fill_between(dates, il_values, 0, 
        ...
        ax2.fill_between(dates, net_values, 0,
                         where=np.array(net_values) > 0,
                         color=self.colors['profit'], alpha=0.3, label='Net Profit')
        ax2.fill_between(dates, net_values, 0,
                         where=np.array(net_values) < 0,
                         color=self.colors['loss'], alpha=0.3, label='Net Loss')
    ...
    def plot_price_impact_heatmap(self, save_path='price_impact_heatmap.png'):
        ...
        im = ax.imshow(impact_matrix, aspect='auto', cmap='RdYlGn_r', 
                       extent=[trade_sizes[0], trade_sizes[-1], 
    ...
    def plot_risk_score_breakdown(self, il_pct, volatility, range_width, 
                                   days, save_path='risk_breakdown.png'):
        """
        ...
        colors_list = [self.colors['loss'], self.colors['highlight'], 
                      self.colors['neutral'], self.colors['profit']]
        bars = ax1.bar(range(len(components)), components.values(), 
        ...
        wedges, texts, autotexts = ax2.pie(weighted_scores.values(), 
                                            labels=weighted_scores.keys(),
                                            autopct='%1.1f%%',
                                            colors=colors_list,
                                            startangle=90,
                                            textprops={'fontweight': 'bold'})
        ax2.set_title(f'Weighted Risk Score: {total_score:.1f}/100', 
        ...
        ax2.text(0, -1.3, category, ha='center', fontsize=16, 
    ...
    def plot_scenario_comparison(self, scenarios_data, save_path='scenario_comparison.png'):
        ...
        for i, (bar, val) in enumerate(zip(bars1, il_values)):
            ax1.text(val - 0.3, i, f'{val:.2f}%', va='center', ha='right', 
        ...
        # Plot 3: Net Result
        colors_net = [self.colors['profit'] if v > 0 else self.colors['loss'] 
                     for v in net_values]
        ...
        for i, (bar, val) in enumerate(zip(bars3, net_values)):
            ax3.text(val + (20 if val > 0 else -20), i, f'${val:.0f}', 
        ...
        bars4 = ax4.bar(perf_metrics, values, color=[self.colors['highlight'], 
                       self.colors['profit'], self.colors['neutral']], alpha=0.7)
        ...
        for i, (bar, scenario) in enumerate(zip(bars4, best_scenarios)):
            height = bar.get_height()
            ax4.text(i, height + 50, scenario, ha='center', va='bottom', 
    ...
    def plot_backtest_summary(self, historical_data, save_path='backtest_summary.png'):
        ...
        # Calculate metrics
        initial_price = prices[0]
        il_values = [(2 * np.sqrt(p/initial_price) / (p/initial_price + 1) - 1) * 100 
                     for p in prices]
        ...
        ax1.fill_between(dates, prices, initial_price, 
                        where=np.array(prices) > initial_price,
                        color=self.colors['profit'], alpha=0.2, label='Price Increase')
        ax1.fill_between(dates, prices, initial_price,
                        where=np.array(prices) < initial_price,
                        color=self.colors['loss'], alpha=0.2, label='Price Decrease')
        ...
        ax4.plot(dates, hodl, color=self.colors['highlight'], linewidth=2.5, 
                label='HODL Strategy', linestyle='--')
        ax4.plot(dates, lp_value, color=self.colors['profit'], linewidth=2.5, 
                label='LP Strategy')
        ax4.fill_between(dates, hodl, lp_value,
                        where=np.array(lp_value) > np.array(hodl),
                        color=self.colors['profit'], alpha=0.2, label='LP Outperforms')
        ax4.fill_between(dates, hodl, lp_value,
                        where=np.array(lp_value) < np.array(hodl),
                        color=self.colors['loss'], alpha=0.2, label='HODL Outperforms')

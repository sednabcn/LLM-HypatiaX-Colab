...
class DeFiAdvancedCalculator:
    ...
    def vesting_cliff_linear(
        ...
        if not cliff_passed:
            ...
            vested_amount = min(
                total_tokens * (time_since_cliff / vesting_days),
                total_tokens
            )
...
comp_scenarios = [
    {'borrow_apr': 0.05, 'comp_per_block': 0.5, 'comp_price': 50, 'total_borrowed': 1_000_000, 'desc': 'Low incentives'},
    {'borrow_apr': 0.08, 'comp_per_block': 1.0, 'comp_price': 100, 'total_borrowed': 500_000, 'desc': 'High incentives'},
    {'borrow_apr': 0.03, 'comp_per_block': 2.0, 'comp_price': 75, 'total_borrowed': 1_000_000, 'desc': 'Paid to borrow'},
]
...
for comp in comp_scenarios:
    ...
    scenarios.append({
        'scenario': comp['desc'],
        'formula': 'Compound APY with Rewards',
        **result
    })
    print(f"{comp['desc']}: Borrow {result['borrow_apr']*100:.2f}% - Rewards {result['reward_apy']*100:.2f}% = Net {result['net_apy']*100:.2f}%")
...
for protocol in protocols:
    ...
    print(f"{protocol['name']:<15}: ${protocol['volume']:>12,} vol × {protocol['fee']*100:.2f}% × {protocol['cut']*100:.1f}% = ${revenue:>10,.2f}/day")
...
for cdp in maker_cdps:
    ...
    scenarios.append({
        'scenario': cdp['desc'],
        'formula': 'Maker Stability Fee',
        **result
    })
    print(f"{cdp['desc']}: ${result['principal']:,.0f} → ${result['total_debt']:,.2f} (Fee: ${result['fee_amount']:,.2f})")
...
for scenario in il_scenarios:
    ...
    scenarios.append({
        ...
        'formula': 'IL with Fees Net',
        **result
    })
    profitable_mark = "✓" if result['profitable'] else "✗"
    print(f"{scenario['desc']}: IL {result['il_pct']:.2f}% + Fees {result['fees_earned_pct']:.2f}% = {result['net_result_pct']:.2f}% {profitable_mark}")
...
for perp in perps:
    ...
    scenarios.append({
        'scenario': perp['desc'],
        'formula': 'Perpetual Basis',
        **result
    })
    print(f"{perp['desc']}: {result['premium_pct']:+.2f}% premium, {result['basis_annualized']:+.1f}% annualized - {result['sentiment']}")
...
arbs = [
    {'price_a': 2000, 'price_b': 2010, 'amount': 100, 'fee_a': 30, 'fee_b': 30, 'gas': 50, 'desc': 'Small arb ($10 spread)'},
    {'price_a': 2000, 'price_b': 2050, 'amount': 100, 'fee_a': 30, 'fee_b': 30, 'gas': 50, 'desc': 'Large arb ($50 spread)'},
    {'price_a': 2000, 'price_b': 2005, 'amount': 100, 'fee_a': 30, 'fee_b': 30, 'gas': 100, 'desc': 'Unprofitable (high gas)'},
]

for arb in arbs:
    result = calc.flash_loan_net_profit(
        arb['price_a'], arb['price_b'], arb['amount'],
        arb['fee_a'], arb['fee_b'], arb['gas']
    )
    scenarios.append({
        'scenario': arb['desc'],
        'formula': 'Flash Loan Arbitrage',
        **result
    })
    profitable_mark = "✓" if result['profitable'] else "✗"
    print(f"{arb['desc']}: Gross ${result['gross_profit']:.2f} - Costs ${result['total_costs']:.2f} = ${result['net_profit']:.2f} {profitable_mark}")
...
for vest in vestings:
    ...
    scenarios.append({
        'scenario': vest['desc'],
        'formula': 'Vesting Schedule',
        **result
    })
    print(f"{vest['desc']}: {result['vested_amount']:,.0f} vested ({result['vested_pct']:.1f}%), {result['locked_amount']:,.0f} locked")
...
for col_scenario in collateral_scenarios:
    ...
    print(f"{col_scenario['desc']}: Coverage = {result['coverage_ratio']:.3f} - {result['health_status']} ({result['liquidation_risk']} risk)")
...
for farm in farms:
    ...
    scenarios.append({
        'scenario': farm['desc'],
        'formula': 'Yield Farming ROI',
        **result
    })
    profitable_mark = "✓" if result['profitable'] else "✗"
    print(f"{farm['desc']}: ROI = {result['roi_pct']:.2f}% "
...
def compare_lending_protocols(
    self,
    utilization_range: List[float]
) -> Dict:
    """
...
if __name__ == "__main__":
    ...
    for result in recovery_results:
        ...
        print(f"Price {result['price_ratio']}x: IL {result['il_pct']:.2f}%, "
    ...
    for gas_scenario in gas_scenarios:
        ...
        print(f"{gas_scenario['desc']}: ${result['gas_cost_usd']:.2f} cost, "
    ...
    for ratio, fee_apr, t in [(1.5, 0.20, 0.25), (2.0, 0.30, 0.5)]:
        ...
        print(f"  Ratio {ratio}: IL {res['il_pct']:.2f}% | Fees {res['fees_earned_pct']:.2f}% | Net {res['net_result_pct']:.2f}%")

...
def calculate_il_with_fees(initial_price, current_price, initial_x, initial_y, 
                           days_elapsed, daily_volume_usd, fee_tier=0.003):
    """
    Calculate Impermanent Loss and Fees Earned
    
    Args:
        initial_price: Starting ETH price in USD
        current_price: Current ETH price in USD
        initial_x: Initial ETH amount
...
def backtest_lp_strategy(historical_prices, initial_eth=10, initial_usdc=20000, 
    ...
    for idx, day in enumerate(historical_prices):
        ...
        calc = calculate_il_with_fees(
            ...
            initial_x=initial_eth,
            initial_y=initial_usdc,
            days_elapsed=days_elapsed,
            daily_volume_usd=daily_volume
        )
        
        # HODL value (just holding ETH + USDC separately)
        hodl_value = initial_eth * current_price + initial_usdc
        
        # LP value (constant product formula)
        lp_value_no_fees = 2 * np.sqrt(initial_eth * initial_usdc * current_price)
...
def create_visualizations(df, save_path=png_path):
    ...
    ax1 = axes[0, 0]
    ax1_twin = ax1.twinx()
    ax1.plot(df['date'], df['price'], 'b-', linewidth=2, label='ETH Price')
    ...
    ax2.fill_between(df['date'], df['total_fees'], df['il_usd'], 
    ...
    ax3.fill_between(df['date'], df['hodl_value'], df['lp_value'],
                     where=(df['lp_value'] >= df['hodl_value']),
                     alpha=0.3, color='green', label='LP Wins')
    ax3.fill_between(df['date'], df['hodl_value'], df['lp_value'],
                     where=(df['lp_value'] < df['hodl_value']),
                     alpha=0.3, color='red', label='HODL Wins')
    ax3.set_xlabel('Date')
...
def run_complete_backtest(days=90, initial_eth=10, initial_usdc=20000, 
                         daily_volume=5000000, export_excel=True):
    """
    Run complete backtest with all analysis and visualizations
    """
    
    print("🚀 Starting DeFi LP Backtest Analysis...")
    print(f"Parameters: {days} days, {initial_eth} ETH, ${initial_usdc} USDC")
    print("-" * 60)
    
    # Fetch data
    print("\n1️⃣ Fetching historical ETH prices...")
    historical_prices = get_eth_historical_prices(days)
    ...
    png_path=f'backtest_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
    create_visualizations(results_df,png_path)
    
    # Export to Excel
    if export_excel:
        print("\n5️⃣ Exporting to Excel...")
        excel_path = f'lp_backtest_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'

...
class ImpermanentLossCalculator:
    ...
    def calculate_il_with_fees(self, current_price: float, 
                               ...
                               time_period_days: float = 1) -> Dict[str, float]:
        """
        ...
        return {
            **il_data,
            ...
            'fee_apr': fee_apr,
            'breakeven_volume_multiple': abs(il_absolute) / (float(self.initial_value) * float(self.fee_tier)) if il_absolute < 0 else 0
        }
    
    def generate_il_curve(self, price_range: Tuple[float, float], 
                         steps: int = 50) -> List[Dict[str, float]]:
        """
...
def compare_strategies(initial_investment: float, initial_price: float,
                       current_price: float, fee_tier: float = 0.003,
                       volume_multiple: float = 1.0) -> Dict[str, Dict[str, float]]:
    """
...
if __name__ == "__main__":
    ...
    il_with_fees = calculator.calculate_il_with_fees(
        current_price=4000,
        volume_as_multiple_of_liquidity=10,  # 10x volume
        time_period_days=30
    )
    ...
    print(f"  Hold: ${comparison['hold_strategy']['final_value']:.2f} "
          f"({comparison['hold_strategy']['return_percentage']:.2f}%)")
    print(f"  LP: ${comparison['lp_strategy']['final_value']:.2f} "

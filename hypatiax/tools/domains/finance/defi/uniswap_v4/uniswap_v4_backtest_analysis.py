...
def backtest_v4_strategy(historical_prices, price_lower, price_upper,
                        initial_coin=10, initial_usdc=20000,
                        daily_volume=5000000, base_fee=0.003,
                        enable_hooks=True):
    """
    Run V4 backtest
    ...
    for idx, day in enumerate(historical_prices):
        ...
        v4_calc = calculate_position_metrics(
            initial_price, current_price, price_lower, price_upper,
            initial_coin, initial_usdc, days_elapsed,
            daily_volume, effective_fee_v4
        )
...
def calculate_position_metrics(initial_price, current_price,
                               price_lower, price_upper,
                               initial_x, initial_y,
                               days_elapsed, daily_volume, fee_tier):
...
def create_v4_visualizations(df, coin_symbol, save_path):
    ...
    fig.suptitle(f'Uniswap V4 vs V3 Backtest - {coin_symbol.upper()}', 
    ...
    ax3.fill_between(df['date'], df['v4_fees'], df['v3_fees'],
...
def run_v4_backtest(days=90, initial_coin=10, initial_usdc=20000,
                    price_range_pct=10, daily_volume=50_000_000,
                    coin_symbol='ETH', base_fee=0.003,
                    enable_hooks=True, export_excel=True):
    ...
    results_df = backtest_v4_strategy(
        historical_prices, price_lower, price_upper,
        initial_coin, initial_usdc, daily_volume,
        base_fee, enable_hooks
    )

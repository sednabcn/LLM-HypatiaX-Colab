...
class UniswapV3Position:
    ...
    def _get_amount0_for_liquidity(self, sqrt_price_a: float, sqrt_price_b: float, 
    ...
    def _get_amount1_for_liquidity(self, sqrt_price_a: float, sqrt_price_b: float,
                                    liquidity: float) -> float:
    ...
    def _get_liquidity_for_amount0(self, sqrt_price_a: float, sqrt_price_b: float,
                                    amount0: float) -> float:
    ...
    def _get_liquidity_for_amount1(self, sqrt_price_a: float, sqrt_price_b: float,
                                    amount1: float) -> float:
    ...
    def calculate_liquidity_amounts(self, price_current: float, price_lower: float,
                                    price_upper: float, amount0: float, 
                                    amount1: float) -> Dict[str, float]:
        """
    ...
    def mint_position(self, price_current: float, price_lower: float,
                     price_upper: float, amount0_desired: float,
                     amount1_desired: float) -> Dict:
        """
        ...
        calc = self.calculate_liquidity_amounts(
            price_current, price_lower, price_upper,
    ...
    def get_position_value(self, position_id: int, price_current: float) -> Dict:
        ...
        if in_range:
            range_position_pct = ((price_current - position.price_lower) / 
                                 (position.price_upper - position.price_lower)) * 100
        else:
...
def example_v3_position_management():
    ...
    print(f"  Deposited: {alice_position['amount0_deposited']:.4f} ETH + ${alice_position['amount1_deposited']:.2f} USDC")
    ...
    if alice_position['amount0_refunded'] > 0 or alice_position['amount1_refunded'] > 0:
        print(f"  Refunded: {alice_position['amount0_refunded']:.4f} ETH + ${alice_position['amount1_refunded']:.2f} USDC")
    print()

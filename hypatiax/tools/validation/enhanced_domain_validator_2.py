...
class EnhancedDomainValidator:
    ...
    def _check_sharpe_denominator(self, expr_str: str, expr: sp.Expr, test_data: Optional[Dict], result: Dict) -> Dict:
        """Improved Sharpe ratio denominator checking."""
        expr_clean = expr_str.replace(" ", "").lower()

        # Check string patterns
        sigma_patterns = ["/sigma", "/volatility", "/vol", "/(sigma", "/(volatility", "/(vol"]
        has_sigma_division = any(pattern in expr_clean for pattern in sigma_patterns)

        # Check symbolic division (Pow(-1) = 1/x)
        sigma_vars = ["sigma", "volatility", "vol"]
        sigma_in_denominator = False
        detected_sigma_var = None

        # Check all atoms in expression tree for division patterns
        for atom in sp.preorder_traversal(expr):
            # Direct Pow(-1) check
            if atom.is_Pow and atom.exp == -1:
                base_str = str(atom.base)
                # Check if any sigma variable matches the denominator
                for var in sigma_vars:
                    if var == base_str or var in base_str:
                        sigma_in_denominator = True
                        detected_sigma_var = var
                        break
            # Check Mul nodes for embedded division (a * b^-1)
            elif atom.is_Mul:
        ...
        # Determine which sigma variable to check
        if detected_sigma_var:
            vars_to_check = [detected_sigma_var]
        elif has_sigma_division:
            # String pattern match - check the one that appears in expression
            vars_to_check = [v for v in sigma_vars if self._variable_in_expression(v, expr_str, expr)]
        else:
            vars_to_check = sigma_vars

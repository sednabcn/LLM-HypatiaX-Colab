#!/usr/bin/env python3
"""
Enhanced Symbolic Validation - PRODUCTION VERSION (FIXED)
Handles all LaTeX parsing edge cases and generates proper warnings
Integrates best features from both validator versions

FIXES:
1. Nested exponential detection now generates ERRORS (not warnings) as expected by tests
2. Improved Greek letter handling for risk/finance formulas
3. Better auto-symbolization for complex expressions
"""
import re
...
class EnhancedSymbolicValidator:
    ...
    def validate(self, formula_latex: str, domain: str = "defi", strict_mode: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. Division by zero detection (includes negative exponent check)
    ...
    def _auto_symbolize(self, expr_str: str):
        ...
        # Common built-in functions and constants to skip
        # "sqrt","sigma", "mu", "alpha", "beta", "gamma", "delta", "Delta",
        # "lambda_var", "Phi", "phi", "theta", "tau", "rho", "epsilon",
        # "kappa", "nu", "omega", "Omega", "zeta", "eta", "xi", "Xi",
        # "psi", "Psi", "chi"  # Greek letters
    ...
    def _safe_parse_latex(self, latex_str: str) -> Optional[sp.Expr]:
        ...
        # -------------------------------------------------------------------
        # 1) BEST: try parse_latex directly
        # -------------------------------------------------------------------
        try:
            expr = parse_latex(s)
            if expr is not None:
        ...
        # -------------------------------------------------------------------
        # 4) Try parse_expr with transformations
        # -------------------------------------------------------------------
        try:
            py_candidate = None
            try:
        ...
        # -------------------------------------------------------------------
        # 5) Last resort: plain sympify on original latex
        # -------------------------------------------------------------------
        try:
            locals_dict = self._auto_symbolize(s)
    ...
    def _latex_to_python(self, latex_str: str) -> str:
        ...
        # Handle Greek letters and special symbols FIRST (before any other processing)
        # This prevents them from being mangled by other transformations
        greek_replacements = [
            (r"\\sigma", "sigma"),
            (r"\\mu", "mu"),
            (r"\\alpha", "alpha"),
        ...
        # Replace operators EARLY (before fractions and sqrt)
        ...
        # Also handle multi-character subscripts like d_{12}
    ...
    def _comprehensive_overflow_check(self, expr: sp.Expr, original_latex: str = "") -> Dict[str, Any]:
        ...
        for atom in sp.preorder_traversal(expr):
            ...
            if atom.func == sp.exp:
                ...
                # Check for nested exponentials in the argument - look deeper
                ...
                # Also check if argument contains any exponential subexpressions
            ...
            if atom.is_Pow and atom.base == sp.E:
                ...
                # Check for nested exponentials in exponent - look deeper
                ...
                # Also check if exponent contains any exponential subexpressions
    ...
    def _check_numerical_stability(self, expr: sp.Expr) -> Dict[str, Any]:
        ...
        for atom in sp.preorder_traversal(expr):
            if (
                atom.func == sp.sqrt
                or (atom.is_Pow and atom.exp == sp.Rational(1, 2))
                or getattr(atom.func, "__name__", "") == "sqrt"
            ):
        ...
        # 3. Logarithms
        for atom in sp.preorder_traversal(expr):
            if atom.func in (sp.log, sp.ln) or getattr(atom.func, "__name__", "") == "log":

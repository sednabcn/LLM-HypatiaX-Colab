...
class SymbolicEngine:
    ...
    def _validate_inputs(
        ...
        if X.shape[1] != len(variable_names):
            validation['valid'] = False
            validation['errors'].append(
                f"Variable name count ({len(variable_names)}) doesn't match "
        ...
        if X_max > self.MAX_SAFE_VALUE:
            validation['warnings'].append(
                f"Input X contains extremely large values (max={X_max:.2e}) - overflow risk"
            )
            validation['issues'].append('large_input_values')
        
        if y_max > self.MAX_SAFE_VALUE:
            validation['warnings'].append(
                f"Target y contains extremely large values (max={y_max:.2e}) - overflow risk"
            )
        ...
        if X_min > 0 and X_min < self.MIN_SAFE_VALUE:
            validation['warnings'].append(
                f"Input X contains extremely small values (min={X_min:.2e}) - underflow risk"
            )
    ...
    def _safe_predict(self, X: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            y_pred = np.nan_to_num(y_pred, nan=0.0, posinf=self.MAX_SAFE_VALUE, 
                                   neginf=-self.MAX_SAFE_VALUE)
        ...
        try:
            ...
            if ss_tot < self.EPSILON:
                ...
                result['validation']['warnings'].append(
                    "Target variance near zero - R² score unreliable"
                )
            else:
                r2 = 1 - (ss_res / ss_tot)
    ...
    def _check_expression_stability(
        self, 
        expression: str, 
        variable_names: List[str],
        X: np.ndarray
        ...
        if '/' in expression:
            stability['warnings'].append(
                "Expression contains division - verify denominators are non-zero"
            )
        ...
        if '**' in expression or 'exp(' in expression:
            stability['warnings'].append(
                "Expression contains exponentiation - verify no overflow occurs"
            )
        ...
        if 'log(' in expression:
            stability['warnings'].append(
                "Expression contains logarithm - verify all arguments are positive"
            )
        ...
        if 'sqrt(' in expression:
            stability['warnings'].append(
                "Expression contains sqrt - verify all arguments are non-negative"
            )
...
if __name__ == "__main__":
    ...
    config5 = DiscoveryConfig(
        niterations=20,
        variable_bounds={'x': (0.01, 10)}  # Bounds that data violates
    )

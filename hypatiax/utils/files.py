...
class FilesManager:
    ...
    def _load_entity(self, filename):
        ...
        with resources.path(self.path_dir, '') as model_path:
            return upload_spacy_training_data(str(model_path), filename_, ner_entity)
    ...
    def _load_rules(self, filename):
        if filename.endswith(".jsonl"):
            rules_path = resources.files(self.path_dir).joinpath(filename)
        else:
    ...
    def _load_models(self, filename):
        with resources.path(self.path_dir, filename) as model_path:
            return spacy.load(str(model_path))
